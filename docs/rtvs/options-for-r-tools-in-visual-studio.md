...
- Linting 选项（请参阅 [Linting](linting-r-code.md)）：选择“工具”>“选项”菜单，然后选择“文本编辑器”>”R”>”Lint”
- 高级编辑器选项（[在本文中介绍](#text-editor--r--advanced-options)）：选择“工具”>“选项”菜单，然后选择“文本编辑器”>”R”>“高级”
- 行为选项（[在本文中介绍](#r-tools--advanced-options)）：选择“R 工具”>“选项”菜单，或者“工具”>“选项”，然后滚动到“R 工具”。
...
以下选项控制在[变量资源管理器](variable-explorer.md)和调试器窗口（如监视和局部变量）中处理值的方式（请参阅[调试](debugging-r-in-visual-studio.md)）。
...
| 切换工作区前显示确认对话框 | `Yes` | 如果设置为 `No`，在不同工作区之间切换时，不会出现用户确认提示。 请参阅[在工作区之间切换](r-workspaces-in-visual-studio.md#switching-between-workspaces) |

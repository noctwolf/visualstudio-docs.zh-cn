...
`.Rhistory` 记录用户在 [R 交互](interactive-repl-for-r-in-visual-studio.md)窗口中输入的所有命令。 可以使用“R 工具”>“窗口”>“历史记录”命令打开专用的历史记录窗口。 该窗口具有可用于清除历史记录内容的工具栏按钮和上下文菜单项。
...
| 发布存储过程 | 启动发布 R 脚本中包含的任何存储过程的进程。 请参阅[处理 SQL Server 存储过程](integrating-sql-server-with-r.md#working-with-sql-server-stored-procedures)。 | 
...
| SQL 查询 | 空 `.sql` 文件。 请参阅 [SQL Server 集成](integrating-sql-server-with-r.md)。 |
| 使用 R 的存储过程 | 具有子 SQL 查询和子存储过程模板文件的 R 文件。 请参阅 [SQL Server 集成](integrating-sql-server-with-r.md)。 |

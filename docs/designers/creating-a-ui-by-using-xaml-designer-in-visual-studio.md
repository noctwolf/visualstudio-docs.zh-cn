...
使用`Grid` 轨道可以管理 [网格](/uwp/api/Windows.UI.Xaml.Controls.Grid) 面板中的行和列。 可以创建和删除行和列，并可以调整其相对宽度和高度。 显示在美工板左侧的垂直网格轨道用于行，而显示在顶部的水平线则用于列。
...
边距表示控件边缘与其容器边缘之间的固定空间量。 可使用“属性”窗口中 **“布局”** 下的 [Margin](/uwp/api/windows.ui.xaml.frameworkelement.margin) 属性来设置控件的边距。

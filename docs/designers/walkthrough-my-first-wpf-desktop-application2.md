...
1. 如果正在使用 C#，找到行 `<Window x:Class="ExpenseIt.MainWindow"` 并将其替换为 `<NavigationWindow x:Class="ExpenseIt.MainWindow"`。

     如果正在使用 Visual Basic，找到行 `<Window x:Class=" MainWindow"` 并将其替换为 `<NavigationWindow x:Class="MainWindow"`。

     请注意，将 `<Window` 标记更改为 `<NavigationWindow`时，IntelliSense 也会自动将结束标记更改为 `</NavigationWindow>` 。

    > [!NOTE]
    >  如果更改标记后  “错误列表”窗口打开，那么你可能会看到几个错误。 别担心，在接下来的步骤中做出更改后，这些错误就会消失。

1. 选择 `<Grid>` 和 `</Grid>` 标记，并将其删除。

     NavigationWindow 无法包含“网格”等其他 UI 元素。

1. 在“新建项目”  窗口中，展开“常用” **Common** 类别节点，然后选择“标题”  属性，再输入 `ExpenseIt` 并按“”  键。
...
1. 找到行 `public partial class MainWindow : Window` 并将其替换为 `public partial class MainWindow : NavigationWindow`。
...
1. 在“解决方案资源管理器”中，打开“ExpenseIt”节点的快捷菜单，选择“添加” > “页面”。

1. 在“新建项目”  “添加新项”对话框中，选择  “名称”文本框并输入 `ExpenseItHome`，然后选择“确定”  按钮。
...
1. 在“新建项目”  “添加新项”对话框中，选择  “名称”文本框并输入 `ExpenseReportPage`，然后选择“确定”  按钮。
...
1. 在菜单栏上，依次选择“调试” > “启动调试”（或按 F5）以运行应用程序。
...
1. 将下面 XAML 代码添加到 `Grid` 元素以创建行和列定义：
...
1. 将以下 XAML 代码添加到 `</Grid>` 标记上方，以创建 `Border`、`ListBox` 和 `Button` 控件：
...
1. 生成并运行应用程序。 下图显示了此过程中 XAML 创建的控件的运行时外观。
...
1. 在“解决方案资源管理器”中，打开“ExpenseIt”节点的快捷菜单，选择“添加” > “现有项”。

1. 在“添加现有项”  对话框中，找到刚添加的“watermark.png”  图像，并选中它，然后选择“添加”  按钮。

    > [!NOTE]
    >  你可能需要展开“文件类型”  列表并选择“图像文件” 。

1. 打开“ExpenseItHome.xaml”  文件并将以下 XAML 代码刚好添加到 `</Grid>` 标记上方以创建背景图像：
...
1. 查找行 `<Grid.ColumnDefinitions>` 并将以下内容添加到它的下方：
...
1. 查找行 `<Grid.RowDefinitions>` 并将以下内容添加到它的下方：
...
1. 将 `Grid.Column` 值设置为 1 以将控件移动到第二列。 将每个 `Grid.Row` 值加 1 以将每个控件下移一行。
    ...
    1. 查找行 `<ListBox Name="peopleListBox" Grid.Column="0" Grid.Row="1"`。 将 `Grid.Column="0"` 更改为 `Grid.Column="1"` 并将 `Grid.Row="1"` 更改为 `Grid.Row="2"`。
...
1. 选择 `Button` 元素并将以下 XAML 代码直接添加在 `HorizontalAlignment="Right"` 元素之后：`Click="Button_Click"`。
...
1. 打开“ExpenseItHome.xaml.cs”  或  “ExpenseItHome.xaml.vb”文件。

1. 将下面的代码添加到 `ExpenseItHome` 类中:
...
1. 打开 **ExpenseReportPage.xaml**。
    ...
    此页面在显示“主页”上所选人员的费用报表。

1. 将以下 XAML 代码添加到 `<Grid>` 和 `</Grid>` 标记之间：
...
1. 打开 ExpenseReportPage.xaml，并使用以下 XAML替换 `<Grid>` 和最后一个 `</Grid>` 元素之间的所有内容：
...
1. 在 `<Grid.Resources>` 元素内部，添加以下 XAML 代码：
...
1. 在 C# 中，将以下构造函数添加到 **ExpenseReportPage** 类，或在 Visual Basic 中使用以下内容替换现有类：
...
1. 打开 **ExpenseItHome.xaml.vb** 或 **ExpenseItHome.xaml.cs**。

1. 将 `Click` 事件处理程序代码替换为以下内容：
...
-   [WPF 图形、动画和媒体概述](/dotnet/framework/wpf/graphics-multimedia/index)

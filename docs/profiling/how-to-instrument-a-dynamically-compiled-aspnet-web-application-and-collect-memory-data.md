...
1.  启动探查器。 类型：  
    ...
    |选项|描述|  
    |------------|-----------------|  
    |[/user](../profiling/user-vsperfcmd.md) **:**[`Domain`**\\**]`UserName`|指定拥有 [!INCLUDE[vstecasp](../code-quality/includes/vstecasp_md.md)] 工作进程的帐户的可选域和用户名。 在进程以已登录用户外的用户身份运行时才需要此选项。 名称在 Windows 任务管理器“进程”选项卡上的“用户名”列中列出。|  
...
## <a name="ending-the-profiling-session"></a>结束分析会话  
 若要结束分析会话，请关闭目标 [!INCLUDE[vstecasp](../code-quality/includes/vstecasp_md.md)] Web 应用程序，停止 Internet Information Services (IIS) 以停止分析的进程，然后关闭探查器。 然后重启 IIS。  

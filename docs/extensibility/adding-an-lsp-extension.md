...
语言服务器协议 (LSP) 是一种常见协议，格式为 JSON RPC v2.0，用于提供各种代码编辑器的服务功能的语言。 使用协议时，开发人员可以编写一个单语言版服务器来提供语言服务等 IntelliSense、 错误诊断查找所有引用，以此类推，到支持 LSP 的各种代码编辑器功能。 通过 TextMate 语法文件用于提供基本功能，如语法突出显示或通过编写自定义语言服务使用完整的 Visual Studio 扩展性 Api 集提供的传统上，可以添加 Visual Studio 中的语言服务更丰富的数据。 LSP 的 Visual Studio 支持，有了第三个选项。
...
本文介绍如何创建 Visual Studio 扩展中使用的 LSP 基于语言服务器。 它假定你已在开发基于 LSP 语言服务器，并只是想要将其集成到 Visual Studio。
...
LSP 和 Visual Studio 中的为其支持的目的是不是 Visual Studio 产品的一部分的载入语言服务。 它不是为了扩展现有的语言服务 (如C#) 在 Visual Studio 中。 若要扩展现有的语言，请参阅语言服务扩展性指南 (例如， ["Roslyn".NET 编译器平台](../extensibility/dotnet-compiler-platform-roslyn-extensibility.md))。
...
下表显示了在 Visual Studio 中支持哪些 LSP 功能：
...
> [!NOTE]
> 从 Visual Studio 2017 版本 15.8，对通用语言服务器协议的支持已内置到 Visual Studio。 如果你已经生成 LSP 扩展使用预览[语言服务器客户端 VSIX](https://marketplace.visualstudio.com/items?itemName=vsext.LanguageServerClientPreview)版本，它们将停止工作，一旦升级到 15.8 或更高版本。 你将需要执行以下操作以获取你恢复正常工作的 LSP 扩展：
>
> 1. 卸载 Microsoft Visual Studio 语言服务器协议预览版 VSIX。
>
>    从版本 15.8 开始，每次你执行升级在 Visual Studio 预览版 VSIX 自动检测和清除。
>
...
>
> 4. 请确保在 VSIX 指定 Visual Studio 2017 版本 15.8 预览版 3 作为安装目标的下限。
>
> 5. 重新生成并重新部署。
...
接下来，通过导航到创建的新 VSIX 项目**文件** > **新建项目** > **Visual C#**   >  **扩展性** > **VSIX 项目**:

![创建 vsix 项目](media/lsp-vsix-project.png)

### <a name="language-server-and-runtime-installation"></a>语言服务器和运行时安装

默认情况下，创建要在 Visual Studio 中支持基于 LSP 语言服务器的扩展不包含语言服务器本身或执行它们所需的运行时。 扩展开发人员负责分发语言服务器和所需的运行时。 有几种方法来执行此操作：
...
1. 创建您的扩展插件内名为"语法"的文件夹 （也可以是你选择的任何名称）。

2. 内部*语法*文件夹，包括任何 *\*.tmlanguage*，  *\*.plist*，  *\*.tmtheme*，或 *\*.json*您希望提供自定义着色的文件。

   > [!TIP]
   > 一个 *.tmtheme*文件定义作用域映射到 Visual Studio 分类 （名为颜色密钥） 的方式。 有关指南，可以引用全局 *.tmtheme*中的文件 *%programfiles (x86) %\Microsoft Visual Studio\\\<版本 >\\\<SKU > \Common7\IDE\CommonExtensions\Microsoft\TextMate\Starterkit\Themesg*目录。

3. 创建 *.pkgdef*文件，并添加与此类似的行：
...
> [!NOTE]
> 时，完成前面的步骤后，可采用 NuGet 包的依赖项的 Newtonsoft.Json 和 StreamJsonRpc 包添加到你的项目。 **除非您确信，将 Visual Studio 的版本上安装这些新的版本不更新这些包的扩展定目标**。 程序集不会包含在 vsix 中;相反，它们将提取从 Visual Studio 安装目录。 如果要引用的程序集的版本比在用户的计算机上安装更新，您的扩展插件将无法工作。

你可以然后创建一个新类实现[ILanguageClient](/dotnet/api/microsoft.visualstudio.languageserver.client.ilanguageclient?view=visualstudiosdk-2017)接口，这是所需的语言客户端连接到基于 LSP 语言服务器的主接口。
...
目前，加载 LSP 基于语言服务器扩展插件的唯一方法是通过文件内容类型。 也就是说，定义语言客户端类时 (它实现[ILanguageClient](/dotnet/api/microsoft.visualstudio.languageserver.client.ilanguageclient?view=visualstudiosdk-2017))，将需要定义类型的文件，当打开时，将导致你的扩展加载。 如果打开的不匹配定义的内容类型的文件，将不加载你的扩展。
...
在上述示例中，为以结尾的文件创建的内容类型定义 *.bar*文件扩展名。 内容类型定义给定名称"bar"，并且必须派生自[CodeRemoteContentTypeName](/dotnet/api/microsoft.visualstudio.languageserver.client.coderemotecontentdefinition.coderemotecontenttypename?view=visualstudiosdk-2017)。
...
1. 添加 JSON 文件 (例如， *MockLanguageExtensionSettings.json*) 到项目，其中包含的设置和它们的默认值。 例如：
...
2. 右键单击 JSON 文件并选择**属性**。 更改**构建**操作保存到"内容"和"包含在 VSIX 中的属性设置为**true**。
...
5. 右键单击.pkgdef 文件，然后选择**属性**。 更改**构建**操作**内容**并**包含在 VSIX**属性设置为**true**。
...
依赖于对基于 LSP 的语言在 Visual Studio 中的服务器的支持[打开文件夹功能](https://devblogs.microsoft.com/visualstudio/open-any-folder-with-visual-studio-15-preview/)，专为不需要自定义项目系统。 您可以构建按照你自己自定义项目系统[此处](https://github.com/Microsoft/VSProjectSystem)，但某些功能，例如设置，可能无法正常工作。 LSP 语言服务器的默认值初始化逻辑是文件夹的传入当前正在打开的根文件夹位置，因此，如果使用自定义项目系统，可能需要在初始化以确保语言服务器可以提供自定义逻辑正常启动。
...
- [添加其他语言的 Visual Studio 编辑器支持](../ide/adding-visual-studio-editor-support-for-other-languages.md)
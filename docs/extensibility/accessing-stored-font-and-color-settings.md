...
ms.locfileid: "56335449"
---
# <a name="access-stored-font-and-color-settings"></a>访问存储的字体和颜色设置

Visual Studio 集成的开发环境 (IDE) 将存储修改字体和颜色在注册表中的设置。 可以使用<xref:Microsoft.VisualStudio.Shell.Interop.IVsFontAndColorStorage>界面来访问这些设置。
...
默认情况下未启用事件生成。 若要启用事件生成，类别必须打开使用[__FCSTORAGEFLAGS。FCSF_PROPAGATECHANGES](<xref:Microsoft.VisualStudio.Shell.Interop.__FCSTORAGEFLAGS.FCSF_PROPAGATECHANGES>)。 打开一个类别使 IDE 调用适当<xref:Microsoft.VisualStudio.Shell.Interop.IVsFontAndColorEvents>VSPackage 实现的方法。

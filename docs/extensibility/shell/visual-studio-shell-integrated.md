...
|数据|-   **服务器资源管理器**(简体： 仅限数据)。 请参阅备注 1。<br />-   **数据源**窗口<br />的组完整的数据控件<br />XML 编辑器<br />数据绑定到本地数据源 (。MDF 或。MDB)<br />数据绑定到对象<br />数据绑定到 Web 服务<br />数据绑定到本地数据库服务器<br />数据绑定到远程数据库服务器<br />-远程数据的 DDL 工具<br />-   **服务器资源管理器**扩展性 ([!INCLUDE[vsipsdk](../includes/vsipsdk_md.md)]示例)|  

 ...
 **验证自定义映像清单**  
 ...
 若要创建自定义清单，我们建议你使用 ManifestFromResources 工具自动生成清单。 若要验证自定义清单，请启动图像库查看器并选择文件 >...设置路径，以打开搜索目录对话框。 该工具将使用搜索目录加载图像的清单，但它还将使用它它们若要查找的.dll 文件包含在清单中，映像因此请务必在此对话框中包括的清单和 DLL 的目录。  

...
7. 若要将本地化的菜单文本，请使用`LocCanonicalName`元素。 有关详细信息，请参阅[字符串](../../extensibility/strings-element.md)元素。  
   ...
   - 组合框：  
   组合框进行了可以使用工具栏的下拉列表。 若要将组合框添加到 UI 中，创建[Combos](../../extensibility/combos-element.md)中的元素`Commands`元素。 然后将添加到`Combos`元素`Combo`添加每个组合框的元素。 `Combo` 元素具有相同的属性和子级用作`Button`元素并且还要`DefaultWidth`和`idCommandList`属性。 `DefaultWidth`属性以像素为单位设置宽度和`idCommandList`属性指向用于填充组合框的命令 ID。 

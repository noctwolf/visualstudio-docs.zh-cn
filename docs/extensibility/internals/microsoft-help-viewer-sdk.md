...
-   回退的品牌包 (Branding_\<区域设置 > 能使用.mshc) 安装帮助查看器 2.3 应用根目录中 (示例： C:\Program Files (x86) \Microsoft Help Viewer\v2.3) 的帮助查看器语言包。  这用于未安装任一产品品牌包的情况下 （已安装任何内容），或者其中已安装的品牌包已损坏。  请注意，使用品牌包应用根回退时，将忽略 （徽标和反馈） 的 Visual Studio 元素。  

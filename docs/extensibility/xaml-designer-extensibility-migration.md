...
从 Visual Studio 2019 版本 16.1 以公共预览版的形式，XAML 设计器支持两个不同的体系结构： 设计器隔离体系结构和较新的图面上隔离体系结构。 在.NET Framework 进程中支持不能在托管的目标运行时需要此体系结构转换。 第三方可扩展性模型迁移到图面上隔离体系结构引入重大更改。 本文概述了所做的更改。

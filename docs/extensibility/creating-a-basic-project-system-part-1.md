...
2.  复制*AssemblyInfo.cs*文件从*属性*文件夹*Projects\SimpleProject*文件夹。  
...
6.  在中**属性**窗口中，将**生成操作**的*AssemblyInfo.cs*， *Program.cs*， *SimpleProject.ico*，并*SimpleProject.myproj*到**内容**，并设置其**包含在 VSIX**属性设置为**True**.  
...
5.  添加到 GUID 属性`SimpleProjectFactory`类。 属性的值是新项目工厂的 GUID。  

...
## <a name="adding-functionality-to-the-text-view"></a>将功能添加到文本视图
 ...
 文本视图的事件通知通过使用提供<xref:Microsoft.VisualStudio.TextManager.Interop.IVsTextViewEvents>接口。 在你的客户端对象来接收到文本视图的更改的通知上实现此接口。 通过使用公开此接口到文本视图<xref:Microsoft.VisualStudio.OLE.Interop.IConnectionPointContainer>文本视图从视图中接收通知的更改上的接口。

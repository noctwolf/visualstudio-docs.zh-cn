...
|*DesignTime*文件夹|包含仅在预运行/调试时所需的文件。 这些可能包括 XML 文档、 库、 标头中，工具箱设计时二进制文件、 MSBuild 项目等<br /><br /> XML 文档会理想情况下，放*\DesignTime*文件夹中，但引用的 XML 文档将继续与 Visual Studio 中的参考文件一起放置。 例如，引用的 XML 文档*\References\\[配置]\\[arch]\sample.dll*将是*\References\\[配置]\\[arch]\sample.xml*，并将该文档的本地化的版本*\References\\[配置]\\[arch]\\[locale]\sample.xml*。|  
...
1.  注册表项中指定它：  
     ...
     **HKLM\Software\Microsoft\Microsoft Sdk\<目标平台 > \v<platform version number>\ExtensionSDKs\<SDKName >\<SDKVersion >**\  
...
3.  *Redist*文件夹： 所需的运行时/调试，应获取打包为用户的应用程序的一部分的文件。 所有二进制文件应放置在下*\redist\\< config\>\\< a c h\>*，以及二进制名称应采用以下格式，以确保唯一性： *]* \<公司 >。\<产品 >。\<用途 >。\<扩展 > *。 例如， *Microsoft.Cpp.Build.dll*。 与其他 sdk （例如，javascript、 css、 pri、 xaml、 png 和 jpg 文件） 的文件名可能发生冲突的名称的所有文件应都放置在下 * \redist\\< 配置\>\\< a c h\> \\< sdkname\> \*除了与 XAML 相关联的文件控制。 这些文件应置于下方*\redist\\< config\>\\< a c h\>\\< 组件名称\>\\*。  
...
5.  *配置*文件夹： 三个子文件夹：*调试*，*零售*，并且*CommonConfiguration*。 SDK 作者可以将其下的文件放*CommonConfiguration*时相同的 SDK 文件集应使用，而不考虑针对 SDK 使用者的配置。  
...
2.  ProductFamilyName： 整体 SDK 产品名称。 例如， [!INCLUDE[winjs_long](../debugger/includes/winjs_long_md.md)] SDK 名为"Microsoft.WinJS.1.0"和"Microsoft.WinJS.2.0"属于相同的一系列 SDK 产品系列，"Microsoft.WinJS"。 此属性允许 Visual Studio 和 MSBuild 进行连接。 如果此属性不存在，则 SDK 名称用作产品系列名称。  
...
9. SupportedArchitectures： 分号分隔的 SDK 支持的体系结构的列表。 如果使用的项目中的目标的 SDK 体系结构不受支持，MSBuild 将显示警告。 如果未指定此特性，MSBuild 将永远不会显示此类型的警告。  
...
12. CopyRedistToSubDirectory： 指定的位置下的文件*\redist*文件夹复制相对于应用程序包的根目录 (即**包位置**中选择**创建应用程序打包**向导) 和运行时布局的根元素。 默认位置是应用程序包的根目录并**F5**布局。  

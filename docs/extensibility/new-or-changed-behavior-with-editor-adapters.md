...
### <a name="use-setsite"></a>使用 SetSite()
 必须调用<xref:Microsoft.VisualStudio.OLE.Interop.IObjectWithSite.SetSite%2A>共同创建文本缓冲区，文本视图和代码窗口之前执行任何其他操作在其上的时。 但是，这不是必需的如果您使用<xref:Microsoft.VisualStudio.Editor.IVsEditorAdaptersFactoryService>来创建它们，因为此服务`Create()`方法自身调用<xref:Microsoft.VisualStudio.Shell.Interop.IVsEditorFactory.SetSite%2A>。

### <a name="host-ivscodewindow-and-ivstextview-in-your-own-content"></a>在您自己的内容中托管 IVsCodeWindow 和 IVsTextView
...
#### <a name="use-win32-and-wpf-versions-of-ivscodewindow"></a>使用 Win32 和 WPF 版本的 IVsCodeWindow
...
#### <a name="use-win32-and-wpf-versions-of-ivstextview"></a>使用 Win32 和 WPF 版本的 IVsTextView
 ...
 若要设置为 WPF 模式下文本视图，必须调用<xref:Microsoft.VisualStudio.TextManager.Interop.IVsTextView.Initialize%2A>，并传入[TextViewInitFlags3.VIF_NO_HWND_SUPPORT](<xref:Microsoft.VisualStudio.TextManager.Interop.TextViewInitFlags3.VIF_NO_HWND_SUPPORT>)初始化之一中标记为`InitView`参数。 可以获取<xref:System.Windows.FrameworkElement>通过调用<xref:Microsoft.VisualStudio.Shell.Interop.IVsUIElementPane.CreateUIElementPane%2A>。
...
#### <a name="pass-arrays-as-parameters-in-native-code"></a>将数组作为参数传递在本机代码中
...
#### <a name="eventing-and-undo"></a>事件记录和撤消
 在新的编辑器中，视图将继续执行操作，如呈现和连续引发事件。 即使在打开的撤消群集时才继续操作。 此行为是不同于未执行之前关闭之后撤消群集的这些操作的旧视图。
...
## <a name="unimplemented-methods"></a>未实现的方法
 在文本缓冲区适配器、 文本视图适配器和文本层适配器上还没有实现一些方法。

...
## <a name="package-manifest-schema"></a>包清单架构
 清单 XML 文件的根元素是`<PackageManifest>`。 它有一个属性`Version`, 即清单格式的版本。 如果对该格式进行了重大更改, 则版本格式会发生更改。 本文介绍清单格式版本 2.0, 该格式是通过将`Version`属性设置为 version = "2.0" 的值在清单中指定的。
...
- `InstalledByMsi`-此可选属性指定是否由 MSI 安装此包。 MSI 安装的包由 MSI (程序和功能) 安装和管理, 而不是由 Visual Studio 扩展管理器安装。  默认情况下, 此属性为 "false", 指定 MSI 不安装包。
...
- `<InstallationTarget>`-此元素控制 VSIX 安装程序安装包的位置。 如果该`Scope`属性的值为 "ProductExtension", 则包必须以 SKU 为目标, 该 SKU 已安装清单文件作为其内容的一部分, 以将其可用性播发到扩展。 当特性具有显式或默认值 "ProductExtension" 时, `<InstallationTarget>`元素具有以下特性: `Scope`
...
- `<Dependency>`元素-此子元素具有以下属性:
  ...
  - `Id`-此属性必须是依赖包的唯一 ID。 此标识值必须与此`<Metadata><Identity>Id`包所依赖的包的属性相匹配。 `Id`属性遵循命名空间约定:Company.Product.Feature.Name。 属性只能包含字母数字字符, 且限制为100个字符。
...
- `<Asset>`-此元素包含以下属性和元素:
  ...
  - `Type`-扩展或此元素表示的内容的类型。 每`<Asset>`个元素必须具有单个`Type`, 但多`<Asset>`个元素可能具有相同`Type`的。 根据命名空间约定, 此特性应表示为完全限定名称。 已知类型为:

...
1. 删除默认的控制。 打开*FirstToolWindowControl.xaml*和删除**单击我 ！** 按钮。
...
1. 在中**解决方案资源管理器**，打开*FirstToolWindowPackage.vsct*。 *.Vsct*文件通过使用 XML 来定义工具窗口中的图形用户界面 (GUI) 元素。
...
2. 将按钮元素内的添加`<Buttons>`部分。 此元素将显示在工具窗口工具栏上，使用**搜索**（放大镜） 图标。
...
## <a name="set-the-default-position-for-the-tool-window"></a>设置工具窗口的默认位置
 接下来，工具窗口在 IDE 中指定的默认位置。 工具窗口的配置信息位于*FirstToolWindowPackage.cs*文件。

1. 在中*FirstToolWindowPackage.cs*，找到<xref:Microsoft.VisualStudio.Shell.ProvideToolWindowAttribute>特性，可以在`FirstToolWindowPackage`类，该类将 FirstToolWindow 类型传递给构造函数。 若要指定默认位置，必须将更多参数添加到下面的示例的构造函数中。
...
1. 按**F5**打开 Visual Studio 的实验性生成的新实例。

2. 上**视图**菜单，依次指向**其他 Windows** ，然后单击**第一个工具窗口**。

    媒体播放器工具窗口应在同一位置中打开**解决方案资源管理器**。 如果仍显示在前面所述的相同位置中，重置窗口布局 (**窗口 / 重置窗口布局**)。

3. 单击按钮 (它具有**搜索**图标) 中的工具窗口。 例如，选择受支持的声音或视频文件， *C:\windows\media\chimes.wav*，然后按**打开**。

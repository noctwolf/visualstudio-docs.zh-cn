...
-   对于较大的缩放级别不为 100%（例如，250%或 350%） 的倍数，缩放与双三次插图图像会导致模糊、 冲蚀用户界面。 更好的效果被获取的第一个缩放到 100%（例如，200%或 300%） 的最大的多个与 NearestNeighbor 图像和使用双三次从该处进行缩放。 请参阅特殊情况： 对于大型 DPI prescaling WPF 图像级别的详细信息。  

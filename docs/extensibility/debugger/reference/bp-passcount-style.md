...
## <a name="members"></a>成员
BP_PASSCOUNT_NONE 指定任何断点传递计数样式。

BP_PASSCOUNT_EQUAL 断点传递计数将样式设置为等于。 当命中断点次数等于传递计数时，将触发断点。

BP_PASSCOUNT_EQUAL_OR_GREATER 将断点传递计数样式设置为相等或更高版本。 当命中断点次数等于或大于传递计数时，将触发断点。

BP_PASSCOUNT_MOD 指定取模传递计数。 例如，如果类型是传递计数`BP_PASSCOUNT_MOD`和传递计数值为 4，断点将触发每次命中的计数是 4 的倍数。

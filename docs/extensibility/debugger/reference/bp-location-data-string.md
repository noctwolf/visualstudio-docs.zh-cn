...
## <a name="members"></a>成员
`pThread` [IDebugThread2](../../../extensibility/debugger/reference/idebugthread2.md)对象，表示该断点发生时所在的线程。

`bstrContext` 在代码内的断点，通常显示调用堆栈上的方法或函数名称的上下文。

`bstrDataExpr` 用户输入的数据字符串以设置断点。

`dwNumElements` 断点发生的数据字符串中的元素数。

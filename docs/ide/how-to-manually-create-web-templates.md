...
3.  创建 XML 文件，并用 vstemplate 文件扩展名将其保存在与项目相同的目录中。 不要将其添加到 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] 的项目中。  
...
8.  将该 .zip 模板文件放入 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] 项目模板目录。 默认情况下，该目录为 \My Documents\Visual Studio Version\My Exported Templates\\。  

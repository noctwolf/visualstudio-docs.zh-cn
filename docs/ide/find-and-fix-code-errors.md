...
您已经找到了编辑器并创建了一些代码。 现在，您要确保代码正常运行。 与大多数 IDE 一样，Visual Studio 通过两个阶段确保代码正常运行：生成代码以捕获并解决项目和编译器错误，以及运行代码以查找运行时和动态错误。

## <a name="build-your-code"></a>生成代码

生成配置有两种基本类型：**调试**和**发布**。 Debug 配置生成较慢、较大的可执行文件，允许更丰富的交互式运行时调试体验。 不应发布 Debug 可执行文件。 Release 配置生成较快、经过优化的可执行文件，此可执行文件适合用于发布（至少从编译器的角度看）。 默认的生成配置是**调试**。

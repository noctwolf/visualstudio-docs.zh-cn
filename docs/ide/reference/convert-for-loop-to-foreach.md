...
- 未在循环内使用迭代计数变量（用作索引的情况除外）来访问项。

- 想要简化代码，并降低初始化表达式、条件以及迭代语句中出现逻辑错误的可能性。
...
如果代码中有 [foreach](/dotnet/csharp/language-reference/keywords/foreach-in) 语句，则可使用此重构将其转换为 [for](/dotnet/csharp/language-reference/keywords/for) 循环。

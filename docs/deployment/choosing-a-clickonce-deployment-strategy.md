...
## <a name="install-from-the-web-or-a-network-share"></a>从 Web 或网络共享安装  
 ...
 如果从 Web 部署应用程序，则可以在使用 URL 激活应用程序时将自变量传递给应用程序。 有关详细信息，请参阅[如何： 检索在联机 ClickOnce 应用程序中的查询字符串信息](../deployment/how-to-retrieve-query-string-information-in-an-online-clickonce-application.md)。 无法将参数传递给使用本文档中描述的任何其他方法激活的应用程序。  
...
>  从技术角度看，应用程序被下载和安装到本地计算机上的应用程序缓存中，就如同 Web 应用程序被下载到 Web 缓存中一样。 与 Web 缓存一样，最后会从应用程序缓存中清理文件。 但是，从用户的角度来看，应用程序是从 Web 或文件共享运行的。  
 ...
 若要启用在此部署策略[!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]，单击**未安装该应用程序**上**安装或运行从 Web**发布向导的页。  
  
 若要手动启用此部署策略，，请更改**安装**部署清单中的标记。 (其值可以是 **，则返回 true**或**false**。 在*Mage.exe*，使用**仅限联机状态**选项**应用程序类型**列表。)  
...
## <a name="install-from-a-cd"></a>从 CD 安装  
 ...
 若要手动启用此部署策略，请更改**deploymentProvider**部署清单中的标记。 (在 Visual Studio 中，此属性公开为**安装 URL**上**发布**项目设计器页。 在*Mage.exe*很**开始位置**。)  

---
title: 部署概述-Visual Studio |Microsoft Docs
ms.custom: ''
ms.date: 06/22/2018
...
对于许多常见的应用类型，可以部署应用程序直接从 Visual Studio 中的解决方案资源管理器。 有关此功能的快速教程，请参阅[初探部署](../deployment/deploying-applications-services-and-components.md)。

![选择发布选项](../deployment/media/quickstart-publish-azure.png)

## <a name="what-publishing-options-are-right-for-me"></a>哪些发布选项适合我？

从 Visual Studio 中，应用程序可以直接发布到以下目标：
...
应用服务在 Azure 的云托管虚拟机上运行，但这些虚拟机是为你而托管的。 应用服务中的每个应用程序将分配一个唯一\*。 azurewebsites.net URL; 所有定价免费层以外层允许向站点分配自定义域名。
...
[Azure 虚拟机 (VM)](https://azure.microsoft.com/documentation/services/virtual-machines/) 可用于在云中创建和管理任意数量的计算资源。 通过负责所有软件和更新的 Vm 上，你可以自定义它们为所需的所需的应用程序。 此外，可以通过远程桌面直接访问虚拟机，只要需要，各服务器就会一直维持分配给它的 IP 地址。

缩放虚拟机托管的应用程序涉及根据需求的更多虚拟机启动并随后部署所需的软件。 利用这种额外的控制能力，可以在全球不同区域以不同方式进行缩放。 例如，如果应用程序要为多个地区办事处的员工提供服务，则可以根据这些地区的员工人数来缩放 VM，从而潜在地降低成本。
...
部署到文件系统意味着只需将应用程序的文件复制到你自己的计算机上的特定文件夹。 这是最常使用的用于测试目的，或如果该计算机也运行服务器以供有限数量的人员将应用程序部署。 如果目标文件夹在网络上共享，那么，通过部署到文件系统能够使 Web 应用程序文件可供其他人访问，这些人随后可将其部署到特定服务器。

任何正在运行服务器的本地计算机可以使应用程序可通过 Internet 或 Intranet 具体取决于如何配置和连接到网络。 （如果将计算机直接连接到 Internet，应特别小心，以免其受到外部安全威胁。）这些计算机由你管理，因此你可以完全控制软件和硬件配置。
...
有关详细信息，请参阅[快速入门-将部署到本地文件夹](quickstart-deploy-to-local-folder.md)

## <a name="custom-targets-iis-ftp"></a>自定义目标 (IIS、FTP)

自定义目标，则应用程序部署到非 Azure 应用服务、 Azure 虚拟机或本地文件系统的目标。 它可以部署到你有权访问的文件系统或任何其他服务器（Internet 或 Intranet），包括其他云服务上的服务器。 它可以与 Web 部署（文件或 .ZIP）和 FTP 配合使用。
...
有关详细信息，请参阅[快速入门-将部署到网站](quickstart-deploy-to-a-web-site.md)

## <a name="next-steps"></a>后续步骤

教程：

- [部署.NET Core 应用程序使用发布工具](/dotnet/core/deploying/deploy-with-vs?toc=/visualstudio/deployment/toc.json&bc=/visualstudio/deployment/_breadcrumb/toc.json)
...
- [将 Node.js 应用程序发布到 Azure 中使用 Web 部署](https://github.com/Microsoft/nodejstools/wiki/Publish-to-Azure-Website-using-Web-Deploy?toc=/visualstudio/deployment/toc.json&bc=/visualstudio/deployment/_breadcrumb/toc.json)

...
1. 在**解决方案资源管理器**，展开**引用** > **分析器**(**依赖关系** >  **分析器**有关.NET Core项目)。
...
> [!NOTE]
> 在.NET Core项目中，如果添加对具有 NuGet 分析器的项目的引用这些分析器将自动添加到依赖项目太。 若要禁用此行为，例如如果依赖项目为单元测试项目中，将标记为私有中的 NuGet 包 *.csproj*或 *.vbproj*引用项目文件：
>
> ```xml

---
title: 禁止显示在 Visual Studio 中的代码分析警告
ms.date: 01/29/2018
...
<xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute>属性是包含 IL 的元数据的托管的代码程序集，在条件属性，仅当在编译时定义 CODE_ANALYSIS 编译符号。
...
> [!NOTE]
> 如果将项目迁移到 Visual Studio 2017 中，您可能会突然会遇到与代码分析警告数过多。 如果您不准备好修复警告，并且想要暂时关闭代码分析，打开项目的属性页 (**项目** > **\<项目 > 属性**) 并转到**代码分析**选项卡。取消选中**生成时启用代码分析**，然后重新生成项目。 或者，可以选择不同的、 较小的规则集以针对代码运行。 请记住启用代码分析重新打开时已准备好修复出现的警告。

...
|批注|描述|
|----------------|-----------------|
|`_Called_from_function_class_(name)`|不是为了单独存在；相反，它是一个旨在与 `_When_` 批注一起使用的谓词。 有关详细信息, 请参阅[指定何时以及在何处应用批注](../code-quality/specifying-when-and-where-an-annotation-applies.md)。<br /><br /> 参数是一个任意字符串, 该字符串也会出现`_Function_class_`在某些函数声明的批注中。 `name`  `_Called_from_function_class_`如果使用`_Function_class_`具有相同值的`name`对当前正在分析的函数进行批注, 则返回非零值; 否则返回零。|
...
|`_Use_decl_annotations_`|可用于函数定义 (也称为函数体), 用于替换标头中的批注列表。  使用`_Use_decl_annotations_`时, 将使用出现在同一函数的范围内的标头上的批注, 就像它们也出现在`_Use_decl_annotations_`具有批注的定义中一样。|

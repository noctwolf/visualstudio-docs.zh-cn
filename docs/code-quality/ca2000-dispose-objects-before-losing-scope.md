...
## <a name="how-to-fix-violations"></a>如何解决冲突
 ...
 请注意，可以使用`using`语句 (`Using`中[!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)]) 来包装实现的对象`IDisposable`。 在这种方式中包装的对象将自动释放在结束时的`using`块。
...
## <a name="example"></a>示例
 默认情况下，[!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)]编译器具有溢出检查的所有算术运算符。 因此，任何 Visual Basic 算术运算可能会引发<xref:System.OverflowException>。 这可能导致的规则，如 CA2000 意外冲突。 例如，以下 CreateReader1 函数将生成 CA2000 冲突，因为 Visual Basic 编译器发出溢出检查的加法运算的可能会引发异常会导致无法释放 StreamReader 的指令。

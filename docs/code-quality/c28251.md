...
警告 C28251： 函数的批注不一致： 此实例具有错误  
 ...
 始终在第一个找到的声明和当前的声明之间进行比较。 如果存在其他声明，则会两个一组分别检查每个声明。 除了配对比较外，当前无法执行其他形式的比较，不过可以确定有两个以上的声明/定义。  *文本*字段上述包含之间存在差异 （相当低的级别） 的两个实例的批注的列表。  

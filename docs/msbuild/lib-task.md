...
|**IgnoreSpecificDefaultLibraries**|可选 **String []** 参数。<br /><br /> 解析外部引用时，从 lib.exe 搜索的库列表中删除指定的库。<br /><br /> 此形参对应于使用 `library` 实参的 lib.exe 的 /NODEFAULTLIB 选项。|  
...
|**ModuleDefinitionFile**|可选 **String** 参数。<br /><br /> 指定模块定义文件 (.def) 的名称。<br /><br /> 此形参对应于使用 `filename` 实参的 lib.exe 的 /DEF 选项。|  
|**名称**|可选 **String** 参数。<br /><br /> 生成导入库时，指定正在为其生成导入库的 DLL 的名称。<br /><br /> 此形参对应于使用 `filename` 实参的 lib.exe 的 /NAME 选项。|  
|**OutputFile**|可选 **String** 参数。<br /><br /> 重写 lib.exe 所创建程序的默认名称和位置。<br /><br /> 此形参对应于使用 `filename` 实参的 lib.exe 的 /OUT 选项。|  

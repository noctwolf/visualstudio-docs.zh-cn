...
1. 生成的解决方案使用代码图**体系结构**菜单。

2. 请考虑应用的代码图筛选器以删除解决方案文件夹和"测试资产"，如果你只想要强制实施中产品代码的依赖关系。

3. 在生成代码映射上，删除"External"节点，或其展开以显示外部程序集，具体取决于是否想要强制实施命名空间依赖项，然后从代码图中删除非必需程序集。

4. 创建解决方案使用的新依赖项关系图**体系结构**菜单

5. 选择代码图上的所有节点 (使用_Ctrl_ + _一个_，或通过按使用橡皮筋选择_Shift_密钥之前单击、 拖动和释放。

6. 拖动并放置，或复制和粘贴，新的依赖项验证关系图的所选元素。

7. 这将显示当前应用程序体系结构。 决定想要的体系结构并相应地修改依赖项关系图。

![生成的代码图的依赖项关系图](media/dependency-validation-01.png)

## <a name="CreateLayers"></a> 从项目中创建层
...
|向层中添加项目|将项目拖到层上。|
|创建新的未链接的层|在中**工具箱**，展开**依赖项关系图**部分中，并将**层**到依赖项关系图。<br /><br /> 若要添加多个层，请双击该工具。 完成后，请选择**指针**工具或按**ESC**密钥。<br /><br /> - 或 -<br /><br /> 打开依赖项关系图的快捷菜单中，选择**外**，然后选择**层**。|
|创建嵌套的层|将现有层拖到另一个层上。<br /><br /> - 或 -<br /><br /> 打开层的快捷菜单中，选择**外**，然后选择**层**。|
|创建包含两个或更多现有层的新层|选择层，打开你选择的快捷菜单，然后选择**组**。|
...
## <a name="EditDependencies"></a> 编辑层和依赖关系，以显示预期的设计
 若要描述你打算对你的系统或预期的体系结构进行的更改，请编辑依赖项关系图：

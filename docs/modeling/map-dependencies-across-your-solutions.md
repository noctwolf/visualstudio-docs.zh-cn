...
ms.locfileid: "34265560"
---
# <a name="map-dependencies-with-code-maps"></a>使用代码图中的映射相关性

可以通过创建代码图来可视化代码之间依赖关系。 代码图可帮助你看到如何代码相互配合，而无需读取通过文件和各行代码。

![查看解决方案中的依赖关系](../modeling/media/codemapsmainintro.png)

若要使用代码图，你需要 Visual Studio Enterprise 或 Professional 版。 专业版中的代码映射功能受到限制稍微比在 Enterprise edition 中。

> [!NOTE]
> 共享 Visual Studio Enterprise 中创建与其他人使用 Visual Studio Professional 的用户的图之前，请确保 （例如隐藏的项、 展开的组和跨组链接） 图上的所有项都是可见。

可以映射这些语言中的代码的依赖关系：

- Visual C# 或 Visual Basic 中的解决方案或程序集 (*.dll*或 *.exe*)

- Visual c + + 项目、 头文件中的本机或托管 C 或 c + + 代码 (*.h*或`#include`)，或二进制文件

- 通过 Microsoft Dynamics AX 的 .NET 模块生成的 X++ 项目和程序集

> [!NOTE]
> 对于除 C# 或 Visual Basic 项目，有较少的选项，启动代码图或将项添加到现有代码图。 例如，不能用鼠标右键单击 C++ 项目的文本编辑器中的对象并将其添加到代码图。 但是，你可以拖放各个代码元素或从文件**解决方案资源管理器**，**类视图**，和**对象浏览器**。

## <a name="install-code-map-and-live-dependency-validation"></a>安装代码图和实时的依赖关系验证

若要在 Visual Studio 2017 年 1 中创建代码图，首先安装**代码图**和**实时依赖项验证**组件：

1. 打开**Visual Studio 安装程序**。 你可以打开它从 Windows 开始菜单中，或在 Visual Studio 中通过选择**工具** > **获取工具和功能**。

1. 选择“各个组件”选项卡。

1. 向下滚动到**代码工具**部分，并选择**代码图**和**实时依赖项验证**。

   ![在 Visual Studio 安装程序的代码图和实时依赖项验证组件](media/modeling-components.png)

1. 选择“修改”。

   **代码图**和**实时依赖项验证**组件开始安装。 你可能需要关闭 Visual Studio。

## <a name="add-a-code-map"></a>添加代码图

可以创建空代码图并将项拖放到它，包括程序集引用、 文件和文件夹，或者你可以生成所有的代码图或解决方案的一部分。

若要添加空代码图：

1. 在“解决方案资源管理器”中，打开顶级解决方案节点的快捷菜单 。 选择**添加** > **新项**。

2. 在**添加新项**对话框下**已安装**，选择**常规**类别。

3. 选择**定向关系图 Document(.dgml)** 模板，然后选择**添加**。

   > [!TIP]
   > 此模板可能不显示按字母顺序，因此向下滚动模板列表的底部如果你未看到它。

   空白的代码图显示在解决方案的**解决方案项**文件夹。

同样，你可以创建新的代码图文件而无需将其添加到你的解决方案，通过选择**体系结构** > **新代码图**或**文件** > **新** > **文件**。

## <a name="generate-a-code-map-for-your-solution"></a>生成你的解决方案代码图

若要查看你的解决方案中的所有依赖项：

1. 在菜单栏上，选择**体系结构** > **生成解决方案的代码图**。 如果自上次您生成后，你的代码未发生更改，则可以选择**体系结构** > **解决方案无需构造生成代码图**相反。
...
4. 但是，这对大型解决方案可能并无作用。 事实上，对于复杂的解决方案，内存限制可能会阻止你展开所有组。 相反，若要在单个节点内进行查看，则将其展开。 将鼠标指针移动到节点的顶部，然后单击出现的 V 形。
   ...
   > [!TIP]
   > 有关更多有关使用代码的详细信息使用鼠标、 键盘和触摸屏输入，请参阅图[浏览和重新排列代码图](../modeling/browse-and-rearrange-code-maps.md)。
...
10. 集中精力处理复杂的解决方案代码图的另一种方式是从现有代码图生成包含选定项的新代码图。 保存**Ctrl**时选择你想要专注于的项，打开快捷菜单，然后选择**从所选内容的新关系图**。
...
假设你有在一些具有挂起更改文件中执行代码评审。 若要查看这些更改中的依赖关系，请从这些文件中创建一个代码图。

   ![显示代码图上的特定依赖关系](../modeling/media/codemapsspecificdependenciesintro.png)

1. 在**解决方案资源管理器**，选择项目、 程序集引用、 文件夹、 文件、 类型或要映射的成员。

   ![选择要映射的项](../modeling/media/codemapsselectinsolutionexplorer.png)

1. 上**解决方案资源管理器**工具栏上，选择**在代码图上显示**![创建新关系图从所选节点按钮](../modeling/media/createnewgraphfromselectedbutton.gif)。 或者，打开一个或一组项的快捷菜单，然后选择**在代码图上显示**。

   此外可以将某些项从**解决方案资源管理器**，**类视图**，或**对象浏览器**，到[新](#add-a-code-map)或映射的现有代码。 若要包含项的父层次结构，请按住**Ctrl**键的同时拖动项，或者使用**包括父级**在代码图工具栏上，若要指定默认操作的按钮。 你还可以将程序集文件从 Visual Studio 外部，如中**Windows 资源管理器**。

   > [!NOTE]
   > 当从共享跨多个应用，如 Windows Phone 或 Microsoft 应用商店项目中添加项时在当前处于活动状态的应用程序项目与图中显示这些项。 如果你将上下文更改为另一个应用程序项目并从共享的项目添加更多项，则这些项现在将与最近活动的应用程序项目一起显示。 你对代码图上的项执行的操作仅适用于共享相同上下文的项。
...
4. 如要查看项，请把项展开。 将鼠标指针移动到项的顶部，然后单击出现的 V 形（向下键）。
   ...
   若要展开所有项，选择其使用**Ctrl**+**A**，然后打开代码图快捷菜单并选择**组** >  **展开**。 然而，如果展开所有组会生成不可用的代码图或产生内存问题，则该选项不可用。

5. 继续展开你感兴趣，直到类和成员级别如有必要的项。

   ![将组展开到类和成员级别](../modeling/media/codemapsexpandtoclassandmember.png)

   若要在地图上查看代码中，但不会显示成员，请单击**重新提取子级**图标![重新提取子级图标](../modeling/media/dependencygraph_deletednodesicon.png)中组的左上角。
...
7. 代码图显示关系。 在此示例中，代码图将显示由调用的方法`Find`方法并在解决方案或外部的位置。
...
- [视频： 了解使用 Visual Studio 2015 代码图的代码中的设计](https://channel9.msdn.com/Events/Visual-Studio/Connect-event-2015/502)]

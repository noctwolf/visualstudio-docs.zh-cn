...
![DSL&#45;EMD&#95;的详细信息](../modeling/media/dsl-emd_details.png)
...
4. 下**通过创建链接处理合并**，添加两个路径：
   ...
   - 一个路径链接到父模型的新元素。 你需要输入的路径表达式从现有元素向上浏览通过嵌入关系到父模型。 最后，它将向其分配新元素的新链接中指定的角色。 路径是按如下所示：

      `ExampleModelHasElements.ExampleModel/!ExampleModel/.Elements`

   - 另一条路径指向现有元素的新元素。 路径表达式指定的引用关系和要向其分配新元素的角色。 此路径如下所示：
...
6. 测试 DSL:
    ...
    2. 以下几种方式创建新元素：
        ...
        - 在中**示例模型资源管理器**，右键单击根节点，然后单击**添加新示例元素**。
...
1. 创建[!INCLUDE[dsl](../modeling/includes/dsl_md.md)]解决方案使用组件模型模板。
...
10. 生成和运行解决方案。 显示 Visual Studio 的一个新实例。

...
在图工具栏上，选择**布局** > **分析器**，和你想要运行然后分析器：

|**分析器**|**标识节点**|
|------------------|--------------------------------|
|**循环引用分析器**|每个其他节点上也具有循环依赖项。 **注意：** 中的循环依赖项**泛型**时展开的组上图, 不显示组。|
|**查找中心分析器**|位于前 25% 个高度连接的节点当中<br /><br /> **若要隐藏代码图上的所有其他节点**<br /><br /> -打开代码图快捷菜单，选择**高级**，**选择**，**隐藏未选内容**。<br />     代码图会隐藏未选定的节点，并且分析器会将新节点标识为中心。|
|**未引用的节点分析器**|不具有来自任何其他节点的引用。 **注意：** 假设不使用代码验证每之前这些情况。 无法在代码中以静态方式找到某些依赖项，如 XAML 依赖项和运行时依赖项。|

在你应用代码图分析器之后，它们将继续运行。 如果更改代码图，应用的任何分析器将自动重新处理更新的代码图。 若要停止运行分析器，图工具栏上，选择**布局** > **分析器**。 关闭所选的分析器。

...
2. 单击每个模板以查看说明。 选择最近似你想要创建的解决方案。
   ...
   -   选择**类图**定义具有隔离舱形状的 DSL。 隔离舱形状包含项列表。
...
3. 在相应的向导页中输入 DSL 的文件扩展名。 这是包含 DSL 的实例的文件将使用的扩展名。
   ...
   -   选择不与你的计算机（或想要在其中安装 DSL 的任何计算机）中的任何应用程序关联的文件扩展名。 例如， **docx**并**htm**将不可接受的文件扩展名。
...
### <a name="test-the-solution"></a>测试解决方案
 ...
 在 Visual Studio 中，在解决方案资源管理器中的新实例中打开示例文件。 它将打开为关系图，并带有一个工具箱。
...
- 根类下显示在 DSL 定义关系图中，左上**类和关系**。 将它重命名为与 DSL 不同的名称。 例如，名为 DSL **MusicLibrary**可能具有一个名为根类**音乐**。

- 在关系图类的 DSL 定义关系图中，较低的右侧会显示**关系图元素**列。 可能必须滚动到右侧才能看到它。 它通常名为_YourDsl_**关系图**。

- 如果您使用了**任务流**模板，并且想要创建具有泳道的关系图、 保留和重命名的 Actor 域类和 ActorSwimlane 形状。
...
|不直接显示。|由类作为一部分所组成的类。||
 ...
 若要创建域类和在同一时间及其嵌入，请单击**嵌入关系**工具，然后单击父类，，然后单击关系图的空白部分。
 ...
 **将元素名称属性设置。** 选择一个域属性，可用于标识语言资源管理器中的元素。 例如，在 Song 域类中可以选择 Title 域属性。 在中**属性**窗口中，将**元素名称**到`true`。
...
### <a name="tidy-the-dsl-definition-diagram"></a>整理 DSL 定义关系图
 在添加关系时，某些类将显示在多个地方。 若要减少的外观的数量并加宽关系图，用鼠标右键单击目标类的一种关系，然后依次**将树放在此处**。 对于相反的效果，右键单击目标类的关系，然后单击**拆分树**。 如果未看见这些菜单命令，则确保只选中域类。
...
3.  **打开资源管理器。** 在关系图的一端是语言资源管理器窗口中，通常名为*YourLanguage*资源管理器。 如果未看见此窗口，则它可能位于“解决方案资源管理器”下方的选项卡上。 如果找不到它，在**视图**菜单，依次指向**其他 Windows**，然后单击*YourLanguage* **资源管理器**。

     资源管理器将呈现模型的树视图。

4.  **创建新元素。** 右键单击位于顶部的根节点，然后单击**新添**_YourClass_。

     类的新实例显示在语言资源管理器中。

5.  在创建新实例时验证每个实例是否都具有不同名称。 这会仅当已设置**元素名称**域属性上的标志。

6.  **检查域属性。选择，在类的实例与**检查属性窗口。 它应显示在此域类上定义的域属性。
...
2. **添加形状类**以表示模型关系图上的元素。 从以下工具之一拖到 DSL 定义关系图上：

   - **几何形状**提供了一个矩形或椭圆。

   - **图像形状**显示你提供的映像。

   - **隔离舱形状**是一个矩形，它包含一个或多个项列表。
...
3. **如果创建了图像形状定义图像，**。
   ...
   5.  在中**选择图像**对话框框中，单击下的下拉列表菜单**文件名**，选择的映像。

4. **将文本修饰器添加到形状，以显示域属性。**

    若要显示模型元素的名称或标题，将可能需要至少一个文本修饰器。

    右键单击形状类的标头，指向**外**，然后单击**文本修饰器**。 设置名称的修饰器，然后在属性窗口设置其**位置**。
...
6. **将属性映射到文本修饰器。**
   ...
   2. 在中**DSL 详细信息**窗口中，单击**修饰器映射**选项卡。如果没有看到**DSL 详细信息**窗口，然后在**视图**菜单上，指向**其他 Windows** ，然后单击**DSL 详细信息**。 通常需要提升此窗口的顶部以查看它的所有内容。

   3. 选择修饰器的名称。 下**显示属性**，选择域类的属性的名称。 为每个修饰器重复此过程。

       如果你想要显示相关元素的属性，请单击下的下拉树导航器**显示属性的路径**。
...
7. **请创建域类的元素的工具箱项。**
   ...
   1.  在中**DSL 资源管理器**，展开**编辑器**节点及其所有子节点。
   ...
   5.  设置**工具箱图标**会在工具箱中显示为图标。 可以将它设置为新图标或已用于其他工具的图标。

        若要创建一个新图标，打开在 Dsl\Resources**解决方案资源管理器**。 复制并粘贴现有元素工具 BMP 文件之一。 重命名粘贴的副本，然后双击以对其进行编辑。

        返回到 DSL 定义关系图，选择工具，然后在属性窗口中单击 **[...]** 中**工具箱图标**。 在中**选择位图**对话框中，选择你。从下拉列表菜单 BMP 文件。

   有关详细信息，请参阅[几何形状的属性](../modeling/properties-of-geometry-shapes.md)并[图像形状属性](../modeling/properties-of-image-shapes.md)。
...
4. **创建形状**通过将从工具拖到模型关系图。

5. **验证是否每个文本修饰器出现**并且：

   1.  你可以编辑它，除非已设置，否则**UI 只读**域属性上的标志。
 ...
 **将域属性添加到此关系。** 在图中，Artist-album 关系具有角色的属性。

 **此关系的允许重复属性设置**如果同一对模型元素之间可以存在多个链接相同的类。 例如，可以允许 Teacher 向相同的 Student 教授多个 Subject。
...
### <a name="define-a-connector-to-display-the-relationship"></a>定义连接符以显示关系
 ...
 请确保在每个项旁边会出现一个复选标记**修饰器**列表。

### <a name="define-a-connection-builder-tool"></a>定义“连接生成器”工具
 在中**DSL 资源管理器**窗口中，展开**编辑器**节点和所有子节点。

 右键单击具有与你的 DSL，相同的名称的节点，然后单击**添加新连接工具**。
...
6. **验证不能创建不适当的类之间的连接。** 例如，如果 Album 和 Artist 之间的关系，验证你无法将 Artist 链接到 Artist。
...
1.  **创建容器域类**。 单击**嵌入关系**工具、 单击该模型的根类，然后单击 DSL 定义关系图的空白部分。 这将创建在示例图中名为 Album 的域类。
     ...
     将域属性，如名称添加到类，并设置其**元素名称**属性窗口中的标志。

2.  **创建列表项域类**。 单击**嵌入关系**工具、 容器类 (Album)，然后单击关系图的空白部分。 这将创建在示例图中名为 Song 的域类。

     添加标题改为类，如域属性，并设置其**元素名称**标志。

     添加其他域属性。

     为想要显示的每个列表添加其他列表项域类。

3.  **若要混合使用多种类型的列表中的项**，创建继承自列表类的类。 使列表类成为抽象状态，通过设置其**继承修饰符**。

     例如，如果希望古典音乐按作曲家而不是艺术家进行排序，则可以创建 Song 的两个子类：ClassicalSong 和 NonClassicalSong。

4.  **创建隔离舱形状**。 从拖动**隔离舱形状**工具拖到 DSL 定义关系图上的。

     添加文本修饰器并设置其名称。

     添加隔离舱并设置其名称。

5.  若要允许用户隐藏列表隔离舱，请右键单击隔离舱形状类，指向**外**，然后单击**展开/折叠修饰器**。 在“属性”窗口中，设置修饰器的位置。

6.  单击**关系图元素映射**工具、 容器域类，并单击隔离舱形状。

7.  选中域类和形状之间的关系图元素映射链接。 在中**DSL 详细信息**窗口：
    ...
    2.  单击**隔离舱映射**选项卡。
         ...
         下**显示元素集合路径**，导航到列表元素类 (Song)。 单击下拉箭头以使用导航器工具。
...
2.  在中**DSL 资源管理器**，展开**编辑器**节点及其所有子节点。
...
6.  设置**工具箱图标**会在工具箱中显示为图标。 可以将它设置为新图标或已用于其他工具的图标。

     若要创建一个新图标，打开在 Dsl\Resources**解决方案资源管理器**。 复制并粘贴现有元素工具 .BMP 文件之一。 重命名粘贴的副本，然后双击以对其进行编辑。

     返回到 DSL 定义关系图，选择工具，然后在属性窗口中单击 **[...]** 中**工具箱图标**。 在中**选择位图**对话框框中，从下拉列表菜单中选择 BMP 文件。
...
1.  **将隔离舱添加到隔离舱形状**。 在 DSL 定义关系图中，右键单击隔离舱形状类、 指向**外**，然后单击**隔离舱**。

2.  设置**显示元素集合路径**以导航到该链接，而不是其目标元素。 单击下拉菜单并使用树视图来选择引用关系而不是其目标。 在此示例中，关系是**ArtistAppearedOnAlbums**。

3.  设置**显示属性的路径**若要从链接导航到目标元素。 在示例中，这是**艺术家**。

4.  设置**显示属性**到目标元素，例如的相应属性**名称**。
...
### <a name="domain-enumerations-and-literals"></a>域枚举和文本
 ...
 若要添加域枚举，请右键单击模型中的根**DSL 资源管理器**，然后单击**添加新域枚举**。 元素将显示在**DSL 资源管理器**下**域类型**节点。 此元素不会显示在关系图上。

 若要将枚举文本添加到域枚举，右键单击域枚举中的**DSL 资源管理器**，然后单击**添加新枚举文本**。

 默认情况下，一次只能将具有枚举类型的属性设置为一个枚举值。 如果你希望用户和程序员能够设置的值的任意组合"位域的"设置**IsFlags**枚举的属性。

### <a name="external-types"></a>外部类型
 如果您找不到类型设置域属性的类型时要**类型**下拉列表中，您可以添加外部类型。 例如，可以添加**System.Drawing.Color**到列表的类型。
...
| 问题 | 建议 |
|-|-|
| 在 DSL 定义文件中进行的更改不起作用。 | 单击**转换所有模板**在工具栏中更高版本的解决方案资源管理器，然后重新生成解决方案。 |
| 形状显示了修饰器的名称而不是属性值。 | 设置修饰器映射。 在 DSL 定义关系图上，单击“关系图元素映射”，它是域类和形状类之间的灰色线条。<br /><br /> 打开**DSL 详细信息**窗口。 如果您看不到，在视图菜单上，指向**其他 Windows**，然后单击**DSL 详细信息**。<br /><br /> 单击**修饰器映射**选项卡。选择修饰器的名称。 确保选中它旁边的框。 下**显示属性**，选择域属性的名称。<br /><br /> 有关详细信息，请参阅[关系图上的形状](#shapes)。 |
...
| 不会显示 DSL 的工具箱，但并未更改程序集名称。<br /><br /> 或者，显示一个报告加载扩展失败的消息框。 | 重置实验实例，并重新生成解决方案。<br /><br /> 1.在 Windows 开始菜单下**所有程序**，展开[!INCLUDE[vssdk_current_long](../misc/includes/vssdk_current_long_md.md)]，然后**工具**，然后单击**重置 Microsoft Visual Studio 实验实例**。<br />2.在“生成”菜单上，单击“重新生成解决方案”。 |

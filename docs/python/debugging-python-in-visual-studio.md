...
| 命令 | 自变量 | 说明 |
| --- | --- | --- |
| `$continue`中， `$cont`中， `$c` | 从当前语句开始运行程序。 |
...
| `$procs` | | 列出当前正在调试的进程。 |
| `$stepin`中， `$step`中， `$s` | 如果可能，单步执行下一个函数调用。 |
| `$stepout`中， `$return`中， `$r` | 跳出当前函数。 |
| `$stepover`中， `$until`中， `$unt` | 转到下一个函数调用。 |
| `$thread` | | 显示当前的线程 ID。 |
| `$thread` | 线程 ID | 将当前线程切换为指定线程 ID。 |
| `$threads` | | 列出当前正在调试的线程。 |
| `$up`, `$u` | | 在堆栈跟踪中将当前帧上移一级。 |
| `$where`中， `$w`中， `$bt` | 列出当前线程的帧。 |

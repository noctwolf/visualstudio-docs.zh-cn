...
- `DjangoWeb`（Django 项目）文件夹包含典型的 Django 项目文件：`__init.py__`、`settings.py`、`urls.py` 和 `wsgi.py`。 通过使用项目模板，已经为应用和数据库文件配置了 `settings.py`，且为 `urls.py` 配置了到所有应用程序页（包括登录窗体）的路由。

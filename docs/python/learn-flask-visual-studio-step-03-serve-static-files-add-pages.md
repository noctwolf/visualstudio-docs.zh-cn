...
开发 Flask 应用时，通常会添加多个 Python、HTML、CSS 和 JavaScript 文件。 对于每个文件类型（以及诸如 `web.config` 等其他需要部署的文件），Visual Studio 提供了方便的[项模板](python-item-templates.md)来帮助你入门。
...
当需要通过代码（例如通过 API 终结点实现）提供静态文件时，Flask 提供一种轻松方法，让你使用名为 `static` 的文件夹（位于项目根目录）中的相对路径来引用文件。 本步骤的第二个部分使用简单的静态数据文件演示该方法。

在任一情况下，都可按照个人喜好整理 `static` 中的文件。

### <a name="use-a-static-file-in-a-template"></a>使用模板中的静态文件

1. 在“解决方案资源管理器”中，右键单击 Visual Studio 项目中的“HelloFlask”文件夹，选择“添加” > “新文件夹”，然后命名 `static` 文件夹。
...
1. 将应用的 `templates/index.html` 文件的内容替换为以下代码，此操作会将步骤 2 中使用的 `<strong>` 元素替换为引用 `message` 样式类的 `<span>`。 通过此方式使用样式类为设计元素样式提供了更多灵活性。
...
1. 在“解决方案资源管理器”`static`中，右键单击 Visual Studio 项目中的“HelloFlask”文件夹，选择“添加” > “新文件夹”，然后命名 `static` 文件夹。
...
答：可以在 `static` 文件夹中以你所需的方式添加其他 CSS、JavaScript 和 HTML 文件。 组织静态文件的一种典型方法是创建名为 `fonts`、`scripts` 和 `content` 的子文件夹（用于样式表和任何其他文件）。
...
1. 在“解决方案资源管理器”中，右键单击 `templates` 文件夹，选择“添加” > “新项”，选择“HTML 页”项模板，将文件命名为 `about.html`，然后选择“确定”。

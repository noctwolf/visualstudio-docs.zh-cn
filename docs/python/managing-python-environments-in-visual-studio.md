...
在 Windows 上的 Visual Studio 中，你可在 [Python 环境](#the-python-environments-window)窗口（如本文中所述）中管理这些环境并选择其中一个作为新项目的默认环境。 对于任何给定的项目，你也可以选择特定环境而不使用默认环境。
...
粗体显示的默认环境是 Python 3.6，Visual Studio 可将其用于任何新项目。 列出的任何类型的任何环境均可用作默认环境。

该窗口下半部分中的命令将应用于所选的解释器，而你所看到的其实是 `C:\Python36-32` 中的特定安装（加粗的默认环境是 Anaconda 安装的一部分）。 如果看不到预期的环境，请参阅[手动标识现有环境](#manually-identify-an-existing-environment)。

所列出的每个环境右侧的控件可为此环境打开交互窗口。 （在 Visual Studio 2017 15.5 及更早版本中，可能还会显示另一个控件，用于刷新此环境的 IntelliSense 数据库。 请参阅[环境窗口引用](python-environments-window-tab-reference.md#intellisense-tab)获取有关数据库的详细信息）。
...
可在上图中看到此特定计算机上的完整环境列表，以及用于创建环境的其他命令。

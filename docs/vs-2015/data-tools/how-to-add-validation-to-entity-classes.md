...
4.  对于 Visual Basic 项目：  
    ...
    1.  由于 C# 项目不会自动生成的事件处理程序，您可以使用 IntelliSense 创建分部`Update` *CLASSNAME*方法。  
  
    2.  键入 `partial` 和空格以访问可用分部方法的列表。 单击要为其添加验证的类的更新方法。 下面的代码类似于您选择时生成的代码`Update` *CLASSNAME*分部方法：  

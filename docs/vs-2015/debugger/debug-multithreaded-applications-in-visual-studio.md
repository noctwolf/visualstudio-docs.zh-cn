...
线程是操作系统向其分配处理器时间的指令序列。 在操作系统中运行的每个进程都包含至少一个线程。 包含多个线程的进程称为多线程。
 ...
 Visual Studio 提供了**线程**窗口、 GPU 线程窗口、 并行监视窗口和其他功能，使多线程调试。 了解线程界面功能的最佳方法是执行演练。 请参阅[演练：调试多线程应用程序](../debugger/walkthrough-debugging-a-multithreaded-application.md)和[演练：调试 c + + AMP 应用程序](http://msdn.microsoft.com/library/40e92ecc-f6ba-411c-960c-b3047b854fb5)。
...
- 使用跟踪点可以在不中断的情况下跟踪程序的执行。 对于研究死锁之类的问题，这一点十分有用。
  ...
  [针对旧代码 （Visual c + +） 的多线程处理支持](http://msdn.microsoft.com/library/24425b1f-5031-4c6b-aac7-017115a40e7c)针对使用 MFC 的 c + + 程序员的线程处理概念和示例代码。

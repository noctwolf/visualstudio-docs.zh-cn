...
ms.locfileid: "47469652"
---
# <a name="graphics-pipeline-stages"></a>图形管道阶段
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

本主题的最新版本，请参阅[图形管道阶段](https://docs.microsoft.com/visualstudio/debugger/graphics/graphics-pipeline-stages)。  
  
“图形管道阶段”窗口可帮助了解 Direct3D 图形管道的每个阶段如何转换各个绘图调用。  
  
 这是“管道阶段”窗口：  
  
 ![3&#45;D 对象也会通过管道阶段。](../debugger/media/gfx-diag-demo-pipeline-stages-orientation.png "gfx_diag_demo_pipeline_stages_orientation")  
...
-   在中**图形管道阶段**窗口中，找到的着色器对应的着色器阶段你想要检查。 然后，在预览图像下，按照着色器阶段标题链接 — 例如，链接**顶点着色器 obj:30**若要查看顶点着色器源代码。  
  
    > [!TIP]
    >  对象编号**obj:30**，用于标识此着色器在整个图形分析器界面中此类如对象表和像素历史记录窗口中所示。  
  
##### <a name="to-debug-a-shader"></a>调试着色器  
  
-   在中**图形管道阶段**窗口中，找到的着色器对应的着色器阶段要进行调试。 然后，在预览图像下，选择**启动调试**。 进入 HLSL 调试器的此入口点默认为着色器对相应阶段的首个调用 — 即着色器在此绘图调用过程中处理的第一个像素、顶点或基元。 可以通过访问此着色器对特定像素或顶点的调用**图形像素历史记录**。  
 ...
 **输入装配器**  
 ...
 在“管道阶段”窗口中，“输入装配器”输出会可视化为框架模型。 若要深入研究一下结果，请选择**输入装配器**中**图形管道阶段**窗口，以使用模型编辑器的完整三维查看组合的顶点。  
  
> [!NOTE]
>  如果`POSITION`语义不显示在输入装配器输出中，则不会显示**输入装配器**阶段。  
  
 **顶点着色器**  
 顶点着色器阶段会处理顶点，通常执行转换、外观设置和照明等操作。 顶点着色器生成的顶点数与它们作为输入的数量相同。  
  
 在“管道阶段”窗口中，顶点着色器输出会可视化为框架光栅图像。 若要深入研究一下结果，请选择**顶点着色器**中**图形管道阶段**windows 图像编辑器中查看处理后的顶点。  
  
> [!NOTE]
>  如果`POSITION`或`SV_POSITION`语义不存在在顶点着色器输出中，则不会显示**顶点着色器**阶段。  
 ...
 **几何着色器**  
 ...
 在“管道阶段”窗口中，几何着色器输出会可视化为框架光栅图像。 若要深入研究一下结果，请选择**几何着色器**中**图形管道阶段**窗口图像编辑器中查看处理后的基元。  
 ...
 **像素着色器**  
 ...
 在“管道阶段”窗口中，像素着色器输出会可视化为全彩色光栅图像。 若要深入研究一下结果，请选择**像素着色器**中**图形管道阶段**窗口图像编辑器中查看处理后的基元。  
  
 **输出合并器**  
 输出合并器将新呈现的像素的效果与对应缓冲区（颜色、深度和模具）的现有内容合并在一起，以在这些缓冲区中生成新值。  
  
 在“管道阶段”窗口中，输出合并器输出会可视化为全彩色光栅图像。 若要深入研究一下的结果，请选择**输出合并器**中**图形管道阶段**窗口以查看合并的帧缓冲区。  
  
### <a name="vertex-shader-preview"></a>顶点着色器预览  
 当选择中的顶点着色器阶段**图形管道阶段**窗口中，**输入缓冲区**显示面板。 在其中可以找到有关提供给顶点着色器的顶点列表的详细信息（在它们经过输入装配器组合之后）。  
  
 ![顶点着色器阶段输入的缓冲区查看器](../debugger/media/gfx-diag-vertex-shader-inbuffers.png "gfx_diag_vertex_shader_inbuffers")  
  
 若要查看顶点着色器阶段的结果，请选择顶点着色器阶段缩略图，以查看网格的完整大小光栅化框架（经过顶点着色器转换之后）。  
  
 ![顶点着色器阶段结果预览](../debugger/media/gfx-diag-vertex-shader-preview.png "gfx_diag_vertex_shader_preview")  

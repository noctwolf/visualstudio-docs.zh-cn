...
```  
 ...
 现在，打开**实时可视化树**窗口 (**调试 / Windows / 实时可视化树**，或沿 IDE 左侧查找它)。 将其拖离其停靠位置以便我们可以并排查看此窗口和“实时属性”窗口。 在“实时可视化树”窗口中，展开 ContentPresenter 节点。 它应包含按钮和列表框的节点。 展开该列表框（然后展开 ScrollContentPresenter 和 ItemsPresenter）以查找列表框各项。 该窗口应如下所示：  
 ...
 现在让我们看看其中一个列表框项的属性。 选择“实时可视化树”中的第一个列表框项并单击工具栏上的“显示属性”图标。 应显示“实时属性资源管理器”。 请注意，**内容**字段是"Item1"，并**背景**字段是 **#ffffffe0** （浅黄色）。 返回到“实时可视化树”，然后选择第二个列表框项。 **实时属性资源管理器**应显示**内容**字段是"Item2"，并**后台**字段是 **#ffd3d3d3** （浅灰色).  

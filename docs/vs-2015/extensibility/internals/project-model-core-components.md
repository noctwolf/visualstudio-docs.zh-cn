...
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsPersistHierarchyItem2>|使项目以管理其项目项，以作为磁盘或其他存储系统中的对象上的文件的持久性的所有方面。 `IVsPeristHierarchyItem2`接口用于未实现的项<xref:Microsoft.VisualStudio.Shell.Interop.IVsPersistDocData2>接口。|  

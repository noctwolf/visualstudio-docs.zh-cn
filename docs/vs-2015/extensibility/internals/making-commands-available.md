...
## <a name="current-context-and-the-visibility-of-commands"></a>当前上下文和命令的可见性  
 ...
 [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] 监视而导致的用户操作，例如加载项目或编辑转到生成的更改。 发生更改时，会自动修改 IDE 的外观。 下表显示了四种主要上下文的 IDE 更改[!INCLUDE[vsprvs](../../includes/vsprvs-md.md)]监视器。  
  
|上下文的类型|描述|  
|---------------------|-----------------|  
|活动项目类型|对于大多数项目类型，这`GUID`值并作为 VSPackage 实现项目的 GUID 相同。 但是，[!INCLUDE[vcprvc](../../includes/vcprvc-md.md)]项目使用的项目类型`GUID`作为值。|  

...
|"表达式太复杂"时处理设计时模板或编译运行时 （预处理过的） 模板。<br /><br /> [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 尝试检查由运行时模板生成的代码时停止工作。|文本块是太长。 T4 将文本块转换为具有一个字符串的每个模板行文本的字符串串联表达式。 很长的文本块可以超越编译器的大小限制。|如将表达式块的长文本块拆分：<br /><br /> `<#= "" #>`|  

...
## <a name="access-from-tests-to-private-methods-and-variables"></a>从测试到私有方法和变量的访问  
 ...
 仅使用公共项和内部项进行测试  
 编写测试，使其仅使用公共（或内部）类和成员。 这是最佳方法。 即使你重构所测试的程序集的内部实现，你的测试也可继续工作。 通过在做出更改之前和之后应用相同的测试，可以确保你所做的更改没有改变程序集的行为。  
...
```  
 ...
 定义测试接口  
 定义一个接口，该接口包括要测试的类的公共成员和你希望测试可使用的私有成员的其他属性和方法。 将此接口添加到要测试的项目中。 例如：  

---
title: DA0021：第 1 代垃圾回收率高 | Microsoft Docs
ms.date: 11/15/2016
...
|分析方法 |所有 |  
|消息 |没有第 1 代垃圾回收的发生率非常高。 按设计来说，如果分配了大部分程序的数据结构且其保留时间较长，那么这通常并不是问题。 但是，如果此行为不在计划内，则表示应用程序可能锁定了对象。 如果不确定，可以收集 .NET 内存分配数据和对象生存期信息，便于了解应用程序使用的内存分配模式。|  
|规则类型 |信息 |  

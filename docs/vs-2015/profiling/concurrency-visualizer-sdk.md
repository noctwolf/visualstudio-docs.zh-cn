...
在C#中, 通过调用[标记](/previous-versions/hh694099(v=vs.140))类中的方法来使用默认提供程序。 它公开了四种生成标记的方法:[WriteFlag](/previous-versions/hh694185(v=vs.140))、 [EnterSpan](/previous-versions/hh694205(v=vs.140))、 [WriteMessage](/previous-versions/hh694161(v=vs.140))和[WriteAlert](/previous-versions/hh694180(v=vs.140))。 有多个用于这些函数的重载，具体取决于是否想要使用这些属性的默认值。  最简单的重载仅采用指定事件说明的字符串参数。 该说明显示在并发可视化工具报表中。
...
1. 创建[microsoft.concurrencyvisualizer.instrumentation.markerwriter>](/previous-versions/hh694138(v=vs.140))对象。 构造函数采用一个 GUID。
...
1. 若要使用新的[microsoft.concurrencyvisualizer.instrumentation.markerseries>](/previous-versions/hh694127(v=vs.140)), 请先使用[microsoft.concurrencyvisualizer.instrumentation.markerwriter>](/previous-versions/hh694138(v=vs.140))对象创建它, 然后直接从新序列生成标记事件。

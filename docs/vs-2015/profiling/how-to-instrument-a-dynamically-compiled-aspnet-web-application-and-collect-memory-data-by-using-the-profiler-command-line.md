...
本主题的最新版本，请参阅[如何： 使用 Profiler 命令行检测动态编译的 ASP.NET Web 应用程序并收集内存数据](https://docs.microsoft.com/visualstudio/profiling/how-to-instrument-a-dynamically-compiled-aspnet-web-application-and-collect-memory-data-by-using-the-profiler-command-line)。  
...
2.  以典型方式启动 [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] Web 应用程序。  
...
## <a name="ending-the-profiling-session"></a>结束分析会话  
 若要结束分析会话，请关闭目标 [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] Web 应用程序，停止 Internet Information Services (IIS) 以停止分析的进程，然后关闭探查器。 然后重启 IIS。  
...
## <a name="restoring-the-application-and-computer-configuration"></a>还原应用程序和计算机配置  
 完成全部分析后，请替换 web.config 文件，清除分析环境变量并重启计算机，以便将服务器和 [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] 应用程序还原到原始状态。  

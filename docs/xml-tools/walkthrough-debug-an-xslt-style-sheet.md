...
有两个窗口显示输入文档和样式表。 调试器使用这些窗口来显示当前的执行状态。 调试器位于`xsl:if`元素的样式表和中的第一个 book 节点上*books.xml*文件。
...
1.  按 F5 继续。
     ...
     因为第一个 book 节点满足`xsl:if`条件，book 节点添加到**XSL 输出**窗口。 调试器继续执行，直到它再次位于样式表中的 `xsl:if` 元素上。 调试器此时位于上的第二个 book 节点在*books.xml*文件。

     在**监视 1**窗口`self::node()`值更改为第二个 book 节点。 通过检查 price 元素的值，可以确定价格高于平均值，所以，`xsl:if` 条件应失败。

2.  按 F5 继续。

     因为第二个 book 节点不满足`xsl:if`条件，book 节点将不添加到**XSL 输出**窗口。 调试器继续执行，直到它再次位于样式表中的 `xsl:if` 元素上。 调试器现在位于第三个`book`中的节点*books.xml*文件。

     在**监视 1**窗口`self::node()`值更改为第三个 book 节点。 通过检查 `price` 元素的值，您可以确定此价格低于平均价格，因此 `xsl:if` 条件应成立。

3.  按 F5 继续。

     因为`xsl:if`条件未满足，第三个 book 节点将添加到**XSL 输出**窗口。 XML 文档中的所有 book 节点均已处理，调试程序停止。

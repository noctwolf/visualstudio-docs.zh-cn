...
本演练演示如何将数据集和`TableAdapter`到使用离散类库项目的代码**数据集设计器**。 分离数据集和 TableAdapter 代码后，创建[Windows Communication Foundation 服务和 Visual Studio 中的 WCF 数据服务](../data-tools/windows-communication-foundation-services-and-wcf-data-services-in-visual-studio.md)服务以调入数据访问层。 最后，您将创建一个 Windows 窗体应用程序用作表示层。 该层将访问数据服务中的数据。
...
3.  在中**数据源**窗口中，选择**添加新数据源**以启动**数据源配置向导**。
...
## <a name="add-a-reference-to-the-data-entity-and-data-access-tiers-to-the-data-service"></a>将对数据实体的引用和数据访问层添加到数据服务
 由于数据服务需要数据集和 Tableadapter 中的信息，将引用添加到**DataEntityTier**并**DataAccessTier**项目。

### <a name="to-add-references-to-the-data-service"></a>添加对数据服务的引用

1.  右键单击**DataService**中**解决方案资源管理器**然后单击**添加引用**。
...
### <a name="to-set-the-new-presentation-tier-project-as-the-startup-project"></a>若要将新的表示层项目设置为启动项目

-   在中**解决方案资源管理器**，右键单击**PresentationTier**然后单击**设为启动项目**。

## <a name="add-references-to-the-presentation-tier"></a>将引用添加到表示层
 客户端应用程序 PresentationTier 需要对数据服务的服务引用才能访问该服务中的方法。 另外，WCF 服务又需要具有对数据集的引用，才能启用类型共享。 启用类型共享通过数据服务时之前, 添加到数据集分部类的代码不可用到表示层。 因为你通常将添加代码，例如验证代码的行和列更改事件的数据表，则很可能要从客户端访问此代码。
...
1.  在中**解决方案资源管理器**，双击**app.config**文件中**PresentationTier**项目。

2.  找到**maxReceivedMessage**大小特性，然后将值更改为`6553600`。

## <a name="test-the-application"></a>测试应用程序
运行应用程序通过按**F5**。 将数据从`Customers`和`Orders`表从数据服务检索和显示窗体上。

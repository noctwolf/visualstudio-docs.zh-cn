...
通过使用 \*.runsettings 文件，可配置 Visual Studio 中的单元测试。 （只要使用扩展名“.runsettings”，文件名就无关紧要。）例如，可以更改即将运行测试的 .NET Framework 版本、提供测试结果的目录以及在测试运行期间收集的数据。

如果不需要执行任何特殊配置，则无需 \*.runsettings 文件。 \* .runsettings 文件最常见的用途是自定义[代码覆盖率](../test/customizing-code-coverage-analysis.md)。
...
## <a name="example-runsettings-file"></a>示例 .runsettings 文件

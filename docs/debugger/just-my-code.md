...
##  <a name="BKMK_Enable_or_disable_Just_My_Code"></a> 启用或禁用仅我的代码  
 若要启用或禁用仅我的代码，请选择**工具 > 选项**Visual Studio 菜单中的。 在中**调试** > **常规**节点中，选择或清除**启用 ' 仅我的代码**。
  
 ![启用仅我的代码选项对话框中](../debugger/media/dbg_justmycode_options.png "DBG_JustMyCode_Options")  
  
> [!NOTE]
>  **启用 ' 仅我的代码**设置是应用于所有语言中的所有 Visual Studio 项目的全局设置。  
  
###  <a name="BKMK_Override_call_stack_filtering"></a> 在调用堆栈视图中显示非用户代码  
 在视图中显示调用堆栈，如**调用堆栈**并**任务**windows 中，仅我的代码将非用户代码折叠为标记的带批注帧`[External Code]`。 若要查看折叠的帧，请选择**显示外部代码**在调用堆栈的上下文菜单上显示。
...
###  <a name="BKMK_NET_Stepping_behavior"></a> 单步执行行为  
 当您**单步执行**(键盘快捷键： F11) 非用户代码时，调试器将逐步执行代码的下一个用户语句。 当您**单步跳出**(键盘： Shift + F11)，调试器会运行到下一行用户代码。 如果遇到没有用户代码，则执行将继续，直到应用退出、 命中断点，或发生异常。  
...
- `%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers` 文件夹的 `*.natjmc` 文件中指定的函数。  
  ...
  **单步执行**  
  ...
  可以在 `%USERPROFILE%\My Documents\Visual Studio 2017\Visualizers` 中创建自己的 `.natstepfilter` 和 `.natjmc` 以自定义单步执行和调用堆栈窗口行为。  
...
- 若要指定为单个用户的非用户代码，请添加到.natstepfilter 文件`%USERPROFILE%\My Documents\Visual Studio 2017\Visualizers`文件夹。  
...
- 若要指定为单个用户的非用户代码，请添加到.natjmc 文件`%USERPROFILE%\My Documents\Visual Studio 2017\Visualizers`文件夹。  
...
|**UnrelatedCode**|在您的应用程序，但您可能运行的非用户代码不拥有和你的应用程序并不直接依赖它才能正常工作。 （例如，这可能包括广告可以显示广告的 SDK。）在 UWP 项目中，加载到你的应用的 HTTP 或 HTTPS URI 的任何代码也被视为 UnrelatedCode。|  
...
-   如果某个步骤开始在非用户 (**LibraryCode**或**UnrelatedCode**) 代码，随后暂时单步执行行为将如同未启用仅我的代码。 当返回到用户代码，仅我的代码单步单步执行是重新启用。  
  
-   用户代码中的单步执行导致保留当前执行上下文（如对事件处理程序的最后一行进行单步执行）时，调试器会在执行的下一行用户代码处停止。 例如，如果在执行回调**LibraryCode**调试器会继续，直到下的一行用户代码执行的代码。
  
-   **单步跳出**(键盘： Shift + F11) 将在下一行用户代码处停止。 如果遇到没有用户代码，则执行将继续，直到应用退出、 命中断点，或发生异常。  
  
###  <a name="BKMK_JS_Breakpoint_behavior"></a> 断点行为  
  
-   而不考虑该代码的分类将始终会命中在代码中设置断点  

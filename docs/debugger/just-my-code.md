...
<a name="BKMK_CPP_User_and_non_user_code"></a> 仅我的代码是在.NET Framework 和 JavaScript，比 c + + 中不同，因为你可以指定单独的单步执行行为的非用户文件和**调用堆栈**窗口。 

仅我的代码在 c + + 中要考虑仅这些函数为非用户代码：
...
- 单步执行行为：
  ...
  - 中指定的函数 *\*.natstepfilter*中的文件 *%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers*文件夹。  
  
您可以创建 *.natstepfilter*并 *.natjmc*文件以自定义仅我的代码单步执行行为并**调用堆栈**窗口。 请参阅[自定义 c + + 单步执行行为](#BKMK_CPP_Customize_stepping_behavior)并[自定义 c + + 调用堆栈行为](#BKMK_CPP_Customize_call_stack_behavior)。 
...
###  <a name="BKMK_CPP_Customize_stepping_behavior"></a> 自定义 c + + 单步执行行为  

 在 c + + 项目中，可以指定要通过为非用户代码中单步函数 *\*.natstepfilter*文件。  
...
对于 c + + 项目，您可以指定模块、 源文件和函数**调用堆栈**窗口将视为非用户代码通过指定其 *\*.natjmc*文件。  

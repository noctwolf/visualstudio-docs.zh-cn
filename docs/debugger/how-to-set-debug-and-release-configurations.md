...
程序的发布配置进行了完全优化，且不包含任何符号调试信息。 调试信息可以生成.pdb 文件中[具体取决于编译器选项](#BKMK_symbols_release)的使用。 创建.pdb 文件可能非常有用，如果您稍后必须调试发行版本。  
  
有关生成配置的详细信息，请参阅[了解生成配置](../ide/understanding-build-configurations.md)。  
  
你可以将生成配置从**生成**菜单中的，从工具栏中，或在项目属性页。 项目属性页是特定于语言的。 下面的过程演示如何从菜单和工具栏更改生成配置。 有关如何更改中采用不同语言的项目的生成配置的详细信息，请参阅下面的另请参见部分。  
  
## <a name="change-the-build-configuration"></a>更改生成配置  
  
1.  从**构建**菜单中，选择**Configuration Manager**，然后选择**调试**或者**版本**。  
...
## <a name="BKMK_symbols_release"></a>生成符号 (.pdb) 文件

对于大多数项目类型，.pdb 文件生成的默认值为这两个调试和发行版本，但默认设置会有所不同，具体取决于您的特定项目类型和版本的 Visual Studio。 你可以配置是否编译器生成.pdb 文件和要包含的调试信息的种类。
...
在 C# 中的调试配置的项目设置的详细信息，请参阅[项目的 C# 调试配置的设置](../debugger/project-settings-for-csharp-debug-configurations.md)。 对于 Visual Basic，请参阅[本主题](../debugger/project-settings-for-a-visual-basic-debug-configuration.md)。

1. 右键单击解决方案资源管理器中的项目并选择**属性**。

2. 选择**发行**或**调试**中的生成**配置**列表。

2. 选择**构建**设置，然后单击**高级**按钮。

    在 Visual Basic 中，选择**编译**设置和**高级编译选项**按钮。
...
2. 下**链接器 > 调试**，选择所需的选项**生成调试信息**。

    在 c + + 调试配置的项目设置的详细信息，请参阅[项目的 c + + 调试配置的设置](../debugger/project-settings-for-a-cpp-debug-configuration.md)。

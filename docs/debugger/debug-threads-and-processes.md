...
|**并行堆栈**窗口|-对于一个窗口中的多个线程调用堆栈。<br />的每个线程的活动堆栈帧。<br />-调用方和被调用方的任何方法。|-筛选出指定的线程<br />-切换到任务视图<br />-标记或取消标记线程<br />缩放|   
...
|**任务**窗口|-查看有关的信息<xref:System.Threading.Tasks.Task>对象包括任务 ID、 任务状态 （计划的、 正在运行，等待，发生死锁），以及哪个线程分配给任务。<br />-当前调用堆栈中的位置。<br />在创建时传递给任务委托|-切换到当前任务<br />-标记或取消标记任务<br />-冻结或解冻任务|  

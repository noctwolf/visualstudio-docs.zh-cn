...
> [!WARNING]
>  中的调试项目设置**配置属性/调试**适用于 UWP 应用和 c + + 编写的组件类别有不同。 请参阅[启动调试会话 (VB、 C#、 c + + 和 XAML）](../debugger/start-a-debugging-session-for-a-store-app-in-visual-studio-vb-csharp-cpp-and-xaml.md)。  
...
|**远程服务器名称**（远程 Windows 调试器）|指定您要在其上调试应用程序的计算机（不是您的计算机）的名称。<br /><br /> RemoteMachine 生成宏被设置为此属性影响; 的值有关详细信息，请参阅[用于生成命令和属性的宏](/cpp/ide/common-macros-for-build-commands-and-properties)。|  
|**连接**（远程 Windows 调试器）|允许您在远程调试的标准与非身份验证连接类型之间切换。 指定在远程计算机的名称**远程服务器名称**框。 连接类型包括：<br /><br /> -   **带 Windows 身份验证的远程访问**<br />-   **带无身份验证的远程访问**<br /><br /> **请注意**无身份验证的远程调试可能会使远程计算机容易受到安全攻击。 Windows 身份验证模式更安全。<br /><br /> 有关详细信息，请参阅[远程调试安装](../debugger/remote-debugging.md)。|  
...
|**环境**（本地 Windows 调试器和远程 Windows 调试器）|为要调试的程序指定环境变量。 使用标准环境变量语法 (例如， `PATH="%SystemRoot%\..."`)。 这些变量重写系统环境或与系统环境，具体取决于合并**合并环境**设置。 当您单击设置列中时，"编辑..."会显示。 单击该链接可编辑环境变量。|  
...
|**Amp 默认快捷键**|在调试 GPU 代码时，指定默认 AMP 快捷键。 选择**WARP 软件加速器**调查如果问题由硬件或驱动程序而不是你的代码。|  
...
|设置|描述|  
|-------------|-----------------|  
|**生成调试信息**([/debug](/cpp/build/reference/debug-generate-debug-info))|通知链接器包含调试信息，这些信息具有 /Z7、/Zd、/Zi 或 /ZI 指定的格式。|  

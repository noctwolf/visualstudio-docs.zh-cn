---
title: 查看内存中调试器的变量 |Microsoft Docs
...
ms.locfileid: "49919463"
---
# <a name="use-the-memory-windows-in-the-visual-studio-debugger"></a>使用 Visual Studio 调试器中的内存 Windows
...
2. 在中**调试**菜单，依次指向**Windows**。 然后，指向**内存**，然后单击**内存 1**，**内存 2**，**内存 3**，或**内存 4**。 (较低的版本[!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]只有单个**内存**窗口。 如果使用这些版本，只需单击**内存**。)  

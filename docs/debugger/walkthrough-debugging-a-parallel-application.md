...
1.  上**调试**菜单上，指向**Windows** ，然后单击**并行堆栈**。 请确保**线程**在左上角的框中选择。  
     ...
     主线程的调用堆栈显示在一个框中，其他四个线程的调用堆栈则划分到另一个框中。 将这四个线程划分在一起是因为其堆栈帧共享相同的方法上下文；也就是说，它们处于相同的方法中：`A`、`B` 和 `C`。 到视图的线程 Id 和名称的线程的同一框中，将鼠标悬停在带有标头框 (**4 个线程**)。 以粗体显示当前线程。  
...
1.  若要继续执行到命中第二个断点，请在**调试**菜单上，单击**继续**。 下图所示为第二个断点处的线程树。  
     ...
     (Interweaved 行) 的根细线图标指示非当前线程的活动堆栈帧。 在**调用堆栈**窗口中，双击 S.B 可以切换帧。 **并行堆栈**窗口通过使用绿色的曲线的箭头图标来指示当前线程的当前堆栈帧。  
...
1.  若要继续执行到命中第三个断点，请在**调试**菜单上，单击**继续**。  
    ...
    -   **在源中显示线程**切换线程标记在源代码中显示的线程的位置在源代码中的显示。
  
    -   **显示外部代码**即使它们不在用户代码中显示所有帧。 使用此项可查看展开的关系图，其中包含其他帧（这些帧可能因没有相应符号而灰显）。  

2.  在**并行堆栈**窗口中，请确保**自动滚动到当前堆栈帧**已启用工具栏上的按钮。  
...
1.  若要继续执行到命中第四个断点，请在**调试**菜单上，单击**继续**。  
     ...
     **鸟瞰**在大型关系图还有助于**并行堆栈**窗口。 默认情况下，**鸟瞰**上。 但你可以将其切换通过单击窗口右下角的滚动条之间的按钮下图中所示。  
  
     ![鸟瞰&#45;眼睛并行堆栈窗口中的视图](../debugger/media/pdb_walkthrough_5.png "PDB_Walkthrough_5")  
  
     在鸟瞰视图，可以移动矩形可以快速移动到关系图。  
...
5.  上**调试**菜单上，指向**Windows**，然后单击**任务**。 下图显示**任务**窗口。  
...
1.  若要继续执行到命中第二个断点，请在**调试**菜单上，单击**继续**。  
     ...
     以前，**状态**列以前将所有任务都显示为活动，但现在两个任务被阻止。 任务可能因多种不同原因而被阻止。 在**状态**列中，将鼠标悬停在正在等待的任务以了解其阻止原因。 例如，在下图中，任务 3 正在等待任务 4。  
...
1.  若要继续执行到命中第三个断点，请在**调试**菜单上，单击**继续**。  
     ...
     若要更好地直观地显示父-子关系，右键单击列标题行，然后单击**父子视图**。 您应看到以下图示。  
  
     ![父&#45;任务窗口中的子视图](../debugger/media/pdb_walkthrough_9.png "PDB_Walkthrough_9")  
  
     请注意，在同一线程上运行任务 4 和任务 5 (显示**线程分配**如果它隐藏的列)。 此信息不会显示在**线程**窗口中; 在它下面另一个好处的**任务**窗口。 若要确认这一点，查看**并行堆栈**窗口。 请确保你正在查看**任务**。 通过双击它们中查找任务 4 和 5**任务**窗口。 在执行操作时，蓝色突出显示**并行堆栈**窗口将进行更新。 你可以通过上扫描的工具提示来查找任务 4 和 5**并行堆栈**窗口。  
...
1.  若要继续执行到命中第三个断点，请在**调试**菜单上，单击**继续**。 单击**ID**列标题以按 id。 您应看到以下图示。  
     ...
     由于任务 5 已完成，因此不再显示该任务。 如果不是你的计算机上的用例，并且不会显示死锁，通过按步骤一次**F11**。  
  
     任务 3 和任务 4 现在正在相互等待，并且被阻塞。 此外，还存在 5 个作为任务 2 的子级的新任务，目前已计划这些任务。 已计划任务是已在代码中启动但尚未运行的任务。 因此，其**位置**和**线程分配**列为空。  

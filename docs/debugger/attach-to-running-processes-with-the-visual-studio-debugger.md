...
可以在本地或远程计算机上使用 **附加到进程** ，调试正在运行的应用，调试器附加后可同时调试多个进程、 非 Visual Studio 中创建的应用或任何未从 Visual Studio 中启动的应用程序。 例如，如果您正在运行不带调试器的应用，并触发异常，可以将调试器附加到进程中运行应用程序，并开始调试。

有关在 Visual Studio 基本调试的信息，请参阅[调试器入门](../debugger/debugger-feature-tour.md)。
...
2. 在 **可用进程** 列表中，查找并选择想要附加的进程。  

   - 若要快速选择一个进程，请在 **筛选进程** 框键入其名称或首字母。 
  
   - 如果不知道进程名称，请浏览列表或参阅[常见调试方案](#BKMK_Scenarios)，了解一些常见的进程名称。 
  
   >[!TIP]
   > **附加到进程** 对话框处于打开状态时，进程可以在后台启动和停止，因此正在运行的进程列表可能不总是最新的内容。 若要查看当前列表可以选择 **刷新** 。 
...
>[!NOTE]
>可附加到多个应用进行调试，但在调试器中同时只能有一个应用是处于活动状态。 可在 Visual Studio **调试位置** 工具栏或 **进程** 窗口中设置活动的应用。  

##  <a name="BKMK_Attach_to_a_process_on_a_remote_computer"></a>附加到远程计算机上的进程  

您还可以在远程计算机中选择 **附加到进程** 对话框，查看该计算机上运行的可用进程列表，并附加到一个或多个进程以进行调试。 远程调试器 (msvsmon.exe) 必须在远程计算机上运行。 有关详细信息，请参阅[远程调试](../debugger/remote-debugging.md)。 
...
2. 在大多数情况下，**连接类型** 应为 **默认值** 。 在 **连接目标** 框中，使用以下方法之一选择远程计算机：
...
3. 单击 **刷新** 来填充 **可用进程** 列表。
    ...
    >[!TIP]
    > **附加到进程** 对话框处于打开状态时，进程可以在后台启动和停止，因此正在运行的进程列表可能不总是最新的内容。 可随时选择 **刷新** 以查看当前列表。 
...
>[!NOTE]
>可附加到多个应用进行调试，但同时只能有一个应用在调试器中处于活动状态。 可在 Visual Studio 的 **调试位置** 工具栏或 **进程** 窗口中设置活动的应用。  

在某些情况下，在远程桌面 (Terminal Services) 会话中，调试时 **可用进程** 列表不会显示所有可用进程。 如果以具有有限的用户帐户的用户身份运行 Visual Studio 时，**可用进程** 列表不会显示在会话 0 中运行的进程。 会话 0 用于服务和其他服务器进程，包括 *w3wp.exe* 。 你可以通过以下方法解决该问题：使用管理员帐户运行 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] 或从服务器控制台而不是“终端服务”会话运行 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] 。 

如果这两种解决方法都不奏效，第三种方法是通过从 Windows 命令行运行 `vsjitdebugger.exe -p <ProcessId>` 来附加到进程。 可使用 *tlist.exe* 来确定进程 ID 。 若要获取 *tlist.exe* ，请从 [WDK 和 WinDbg 下载](/windows-hardware/drivers/download-the-wdk)中下载并安装 Windows 调试工具。
...
为帮助确定是否使用 **附加到进程** ，以及要附加到哪个进程，下表显示了一些常见的调试方案，并提供了指向尽可能详细的说明链接。 （该列表并不详尽。） 

对于某些应用类型，如通用 Windows 应用 (UWP)，不能直接附加到进程名称，但在 Visual Studio 中代替使用 **调试已安装的应用包** 菜单选项（请参阅表）。
...
对于远程调试方案，必须已在 Visual Studio 中打开了源代码（或源代码的副本）。 远程计算机上已编译的应用二进制文件必须与本地计算机上的来自同一个生成源文件。

某些本地调试的情况下，您可以调试在 Visual Studio 中没有源文件，但是有正确符号文件的应用。 默认情况下，这需要调试版本。 有关详细信息，请参阅[指定符号和源文件](../debugger/specify-symbol-dot-pdb-and-source-files-in-the-visual-studio-debugger.md)。

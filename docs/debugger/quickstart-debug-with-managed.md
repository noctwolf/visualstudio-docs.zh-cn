...
2. 在**Visual C＃**或**Visual Basic**下，选择 **.NET Core** ，然后在中间窗格中选择**控制台应用程序 (.NET Core)**。
...
4. 在*Program.cs*或*Module1.vb*中，替换一下代码
...
在断点处暂停时，将鼠标悬停在语句`c1.AddLast(20)`上，直到绿色**运行单击**按钮![运行单击](../debugger/media/dbg-tour-run-to-click.png "RunToClick")出现，，然后按**运行单击**按钮。
...
1. 单击`c2.First.Value`的第二个实例并更改`c2.First.Value`到`c2.Last.Value`。

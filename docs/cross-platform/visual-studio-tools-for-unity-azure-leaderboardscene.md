---
title: "Visual Studio Tools for Unity Azure 演练 LeaderboardScene | Microsoft Docs"
ms.custom: 
ms.date: 10/19/2017
ms.reviewer: crdun
ms.suite: 
...
---
# <a name="leaderboardscene-explanation"></a>LeaderboardScene 说明

LeaderboardScene 仅由 UI 组成。 在场景层次结构中，**Alt + 单击** Canvas GameObject 旁的**展开箭头**可展开它及其子 GameObject。 Canvas 和 Panel 下嵌套的是附加了 Leaderboard 脚本的 Leaderboard GameObject。

![Leaderboard GameObject](media/vstu_azure-leaderboard-explanation-image1.png)

## <a name="leaderboard-script"></a>Leaderboard 脚本

`Leaderboard` 类使用 `async Start` 函数，该函数仍在脚本启用时进行调用，如同典型 Unity `Start` 函数一样。

`DownloadHighScoresAsync` 使用 <a href="https://msdn.microsoft.com/en-us/library/azure/jj554257(v=azure.10).aspx">`IMobileServiceTable`</a> 的 `OrderBy` 和 `Take` 函数对 Azure 简易表中的高分进行排序，并且仅采用基于 `SizeOfHighScoreList` 常量的顶级项（它们存储在 `highScores` 列表中）。

随后会为列表中的每个项实例化高分行 UI 预设的一个实例。

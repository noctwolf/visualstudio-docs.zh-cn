...
## <a name="opening-unity-scripts-in-visual-studio"></a>在 Visual Studio 中打开 Unity 脚本
...
1. 开始键入 Unity 消息的名称，如 `OnTriggerEnter`。

1. 键入字母“ontri”后，将显示 IntelliSense 建议列表。
...
1. IntelliSense 可插入所选 Unity 消息，包括任何必需的参数：
...
1. 在 Visual Studio 中，将光标放置在你要插入方法的位置，然后按 Ctrl+Shift+M 启动 MonoBehavior 向导。

1. 在“创建脚本方法”窗口中，标记每个要添加的方法的名称旁的复选框。

1. 使用“Framework 版本”下拉列表选择所需的版本。

1. 默认情况下，方法会插入到光标的位置。 或者，可以选择将它们插入到已在类中实现的任何方法后面，方法是将“插入点”下拉列表的值改为所需的位置。

1. 如果需要向导为所选方法生成注释，则标记“生成方法注释”复选框。 这些注释是为了帮助你了解方法的调用时间以及其常规责任。

1. 选择“确定”按钮退出向导，并将方法插入代码中。
...
### <a name="debugging-in-the-unity-editor"></a>在 Unity 编辑器中调试
...
1. 切换到 Unity 并单击“播放”按钮，在编辑器中运行游戏。

  ![在 Unity 中单击“播放”](media/vstu_unity-play-button.png)

1. 当游戏在连接到 Visual Studio 的情况下在 Unity 编辑器中运行时，遇到的任何断点都会中断游戏执行，并在 Visual Studio 中显示游戏遇到断点的代码行。
...
### <a name="debugging-unity-player-builds"></a>调试 Unity 播放器版本
...
1. 在“版本设置”窗口中，标记“开发版本”和“脚本调试”复选框。
...
1. 引用 DLL 项目中正确的 Unity 框架配置文件。 在 Visual Studio 的 DLL 项目属性中，将“目标框架”属性设置为正在使用的 Unity 框架版本。 这是与你的项目作为目标的 API 兼容性相匹配的 Unity 基类库，如 Unity 完整、微型或 Web 基类库。 这可以防止你的 DLL 调用存在于其他框架或兼容性级别中而不存在于你正在使用的 Unity 框架版本中的框架方法。

     ![将 DLL 的目标框架设置为 Unity 框架。](../cross-platform/media/vstu_debugging_dll_target_framework.png "vstu_debugging_dll_target_framework")

1. 将 DLL 复制到 Unity 项目的资产文件夹。 在 Unity 中，资产是与 Unity 应用一起打包和部署的文件，所以可以在运行时加载它们。 由于 DLL 于运行时链接，因而必须将 DLL 作为资产部署。 若要部署为资产，Unity 编辑器需要将 DLL 放置在 Unity 项目的“资产”文件夹中。 可以采用两种方法执行此操作：
...
1. 调试代码。 现在可以同时调试 DLL 源代码以及 Unity 项目的源代码，并使用所有熟悉的调试功能，如断点和单步调试代码。

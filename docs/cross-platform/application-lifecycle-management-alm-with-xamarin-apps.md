...
参考链接：[有关敏捷工具和敏捷项目管理](/vsts/work/backlogs/overview?view=vsts)
...
|[Team Services 中的 Git 入门](/vsts/git/gitquickstart?view=vsts&tabs=visual-studio)|是||
...
参考链接：[生成和发布](/vsts/pipelines/index?view=vsts)

|功能|通过 Xamarin 提供支持|其他注释|
|-------------|----------------------------|-------------------------|
|本地 TFS 服务器|是|生成计算机必须安装 Xamarin，并且能够链接到 OSX 计算机以生成适用于 iOS 的应用。 请参阅[使用 TFVC](/vsts/tfvc/overview?view=vsts)|
|链接到 Visual Studio Team Services 的本地生成服务器|是|请参阅[生成和发布代理](/vsts/pipelines/agents/agents?view=vsts)了解相关介绍。|
|Visual Studio Team Services 承载的控制器服务|是|请参阅 [Build your Xamarin app](/vsts/pipelines/apps/mobile/xamarin?view=vsts&tabs=vsts)（生成 Xamarin 应用）。|
|生成带有前脚本和后脚本的定义|是||
...
|手动测试|是||
|测试管理器（记录和播放测试）|是|仅限 Visual Studio 中的 Windows 设备和 Android 仿真器。 可使用 [Xamarin Test Recorder](/appcenter/test-cloud/uitest/)（Xamarin 测试记录器）记录所有设备。|
|代码覆盖率|n/a||
|[单元测试代码](../test/unit-test-your-code.md)|是|对于 Windows 和 Android 目标，可以使用内置的 MSTest 工具。 若要在 Windows、Android 和 iOS 上运行单元测试，Xamarin 建议使用 NUnit。 请参阅[使用 TFVC](/vsts/tfvc/overview?view=vsts)。|
|[使用 UI 自动化来测试代码](../test/use-ui-automation-to-test-your-code.md)|仅限 Windows|Visual Studio 的 UI 测试记录器仅用于 Windows。 对于所有平台，请参阅 [Xamarin.UITest](/appcenter/test-cloud/uitest/)。|
...
|功能|通过 Xamarin 提供支持|其他注释|
|-------------|----------------------------|-------------------------|
|[分析托管代码的质量](../code-quality/analyzing-managed-code-quality-by-using-code-analysis.md)|是||
...
参考链接：[在 VSTS 和 TFS 中生成和发布](/vsts/pipelines/overview?view=vsts)

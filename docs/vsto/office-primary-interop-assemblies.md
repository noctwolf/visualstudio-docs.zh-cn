...
##  <a name="separateassemblies"></a> 单独的主互操作程序集生成并运行项目  
...
### <a name="primary-interop-assemblies-in-the-program-files-directory"></a>在 program files 目录中的主互操作程序集  
...
-   *%ProgramFiles%\Microsoft visual Studio 12.0\Visual Studio Tools for Office\PIA\Office14*  
     ...
     (或 *%programfiles (x86) %\Microsoft Visual Studio 12.0\Visual Studio Tools for Office\PIA\Office14*在 64 位操作系统上)  
  
-   *%ProgramFiles%\Microsoft visual Studio 12.0\Visual Studio Tools for Office\PIA\Office15*  
  
     (或 *%programfiles (x86) %\Microsoft Visual Studio 12.0\Visual Studio Tools for Office\PIA\Office15*在 64 位操作系统上)  
...
##  <a name="usingfeatures"></a> 在单个项目中使用多个 Microsoft Office 应用程序的功能  
 ...
 在大多数情况下，你应添加对 Visual studio 下 *%ProgramFiles%\Microsoft Visual Studio 12.0\Visual Studio 工具为安装 Office\PIA 的 Pia 引用\*目录。 这些版本的程序集显示在 **“引用管理器”** 对话框的 **“框架”** 选项卡上。 有关详细信息，请参阅[如何： 通过主互操作程序集的目标 Office 应用程序](../vsto/how-to-target-office-applications-through-primary-interop-assemblies.md)。  
...
> [!NOTE]  
>  添加一个引用某些程序集的程序集时，这些被引用的程序集将自动添加到项目中。 例如，引用*Office.dll*和*Microsoft.Vbe.Interop.dll*添加到 Word、 Excel、 Outlook、 Microsoft 窗体或关系图的引用时，将自动添加程序集程序集。  

...
## <a name="access-typed-datasets-in-the-cache"></a>在缓存中的访问类型化数据集  
 如果你想要从 Office 解决方案和外部的应用程序 Office，例如 Windows 窗体应用程序中访问中的类型化数据集的数据或[!INCLUDE[vstecasp](../sharepoint/includes/vstecasp-md.md)]项目中，您必须在单独的程序集引用中定义类型化数据集项目。 将类型化数据集到每个项目添加通过使用**数据源配置**向导或**数据集设计器**，.NET Framework 将处理为不同类型的两个项目的类型化数据集. 有关创建类型化数据集的详细信息，请参阅[创建和配置 Visual Studio 中的数据集](/visualstudio/data-tools/create-and-configure-datasets-in-visual-studio)。  

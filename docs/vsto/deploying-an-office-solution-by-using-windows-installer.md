...
有关如何使用 ClickOnce 部署 Office 解决方案的详细信息，请参阅[通过使用 ClickOnce 部署 Office 解决方案](../vsto/deploying-an-office-solution-by-using-clickonce.md)。  
...
1. 在 **“解决方案资源管理器”** 中，展开 **“OfficeAddInSetup”** 项目节点，然后选择 **“项目助手”** 文件，如下图所示。  
   ...
   ![项目助手文件在解决方案资源管理器](../vsto/media/installshield-projectassistant.png "项目助手文件在解决方案资源管理器")  
...
1. 在 **“解决方案资源管理器”** 中，展开 **“OfficeAddInSetup”** 项目节点，展开 **“指定应用程序数据”** 节点，然后选择 **“可再发行文件”** ，如下图所示。  
   ...
   ![在解决方案资源管理器中的可再发行组件文件](../vsto/media/installshield-redistributablesfile.png "在解决方案资源管理器中的可再发行文件")  
...
3. 在可再发行组件列表中，选中解决方案所针对 .NET Framework 版本的相应复选框。  
   ...
   例如，如果你的解决方案面向 [!INCLUDE[net_v45](../vsto/includes/net-v45-md.md)]，则选择 **“Microsoft .NET Framework 4.5 Full”** 复选框。 可能显示对话框询问是否安装可再发行组件，InstallShield 要求执行此操作才能添加组件作为必备组件。 如果未显示此对话框中，您的计算机上已存在该组件。  
...
10. 从你**桌面**文件夹中，复制*Visual Studio 2010 Tools for Office Runtime.prq*到以下目录之一在计算机上的文件。  
...
12. 在可再发行组件列表中，选中 **“Visual Studio 2010 Tools for Office Runtime”** 复选框。  
   ...
   可能显示对话框询问是否要安装该可再发行组件。 如果未显示此对话框中，则可以跳到[指定你想要在用户计算机上安装解决方案](#Location)本主题的部分。  
...
2. 在 **“项目助手”** 页下方，选择 **“应用程序注册表”** 按钮，如下图所示。  
   ...
   ![应用程序注册表按钮。](../vsto/media/installshield-applicationregistry.gif "的应用程序注册表中按钮。")  
  
   此时将显示 **“应用程序注册表项”** 页。  
  
3. 在 **“是否希望配置应用程序将安装的注册表数据？”** 下，选择 **“是”** 选项按钮。  
  
4. 在中**目标计算机的注册表视图**列表中，添加可支持你想要创建的安装程序类型的密钥层次结构。  
...
5. 创建密钥的层次结构后，打开快捷菜单**SampleCompany.ExcelAddIn**密钥，请选择**新建**，然后选择**字符串值**。  
   ...
   新的字符串值将出现在**目标计算机的注册表数据**列表。 字符串值的名称会突出显示，以便你可以对其进行重命名。  
...
若要修改自定义属性，您可以创建一个用于从文档中移除文档级自定义、 修改相应的属性，然后重新附加到文档的自定义程序。 然后，你可以创建运行该程序的自定义操作，并将该操作添加到你的安装项目中。  
...
11. 在 **“浏览目标文件”** 对话框中，浏览到 **“SetExcelDocumentProperties.Primary.output”** 文件，然后选择 **“打开”** 按钮。  
   ...
   在下一步的几个步骤，将获取该文档的解决方案 ID，然后将该 ID 作为参数传递到控制台应用程序。 你还需要传递文档、 部署清单和文档集的位置。  
...
9. 在菜单栏上依次选择**构建** > **构建 officeaddinsetup**。  
   ...
   在生成完成后，可以找到*setup.exe*的文件**OfficeAddInSetup**项目上的以下位置： _OfficeAddInSetupProjectRoot_ **\OfficeAddInSetup\Express\SingleImage\DiskImages\DISK1\**  

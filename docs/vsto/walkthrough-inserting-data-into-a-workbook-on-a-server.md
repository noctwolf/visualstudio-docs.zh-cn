---
title: 演练： 将数据插入到一台服务器上的工作簿 |Microsoft 文档
ms.custom: ''
ms.date: 02/02/2017
ms.technology:
- office-development
...
---
# <a name="walkthrough-inserting-data-into-a-workbook-on-a-server"></a>演练：将数据插入到服务器上的工作簿中
  本演练演示如何将数据插入 Microsoft Office Excel 工作簿中缓存而不启动 Excel 通过使用数据集<xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument>类。  
...
-   创建<xref:Microsoft.Office.Tools.Excel.ListObject>绑定到工作簿中的数据集。  
...
## <a name="checkpoint"></a>检查点  
 生成并运行 Excel 工作簿项目，以确保它在编译和运行且未发生错误。  
  
#### <a name="to-build-and-run-the-project"></a>生成并运行此项目  
  
1.  在**解决方案资源管理器**，右键单击**AdventureWorksReport**项目，选择**调试**，然后单击**启动新实例**。  
  
     生成该项目，并在 Excel 中打开工作簿。 <xref:Microsoft.Office.Tools.Excel.ListObject>中**Sheet1**为空，因为`adventureWorksLTDataSet`数据缓存中的对象还没有数据。 在下一部分中，你将使用一个控制台应用程序来填充`adventureWorksLTDataSet`对象数据。  
  
2.  关闭 Excel。 不保存更改。  
  
## <a name="creating-a-console-application-project"></a>创建控制台应用程序项目  
 创建要用于在工作簿中缓存的数据集中插入数据的控制台应用程序项目。  
...
## <a name="adding-data-to-the-cached-dataset-by-using-the-console-application"></a>将数据添加到缓存的数据集，使用控制台应用程序  
 使用<xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument>在控制台应用程序，以填充数据的工作簿中缓存的数据集的类。  
...
8.  将以下代码添加到 `Main` 方法中。 此代码声明了以下对象：  
    ...
    -   实例`AdventureWorksLTDataSet`和`ProductTableAdapter`中定义的类型**AdventureWorksDataSet**项目。  
...
9. 以下代码添加到`Main`方法，你在上一步中添加的代码之后。 这段代码执行下列任务：  
    ...
    -   它使用<xref:Microsoft.VisualStudio.Tools.Applications.CachedDataItem.SerializeDataInstance%2A>方法来填充缓存的数据集使用从本地类型化数据集的数据。  
...
10. 在**解决方案资源管理器**，右键单击**DataWriter**项目，指向**调试**，然后单击**启动新实例**。  
     ...
     生成该项目，并填充本地数据集和应用程序将数据保存到工作簿中缓存的数据集时，控制台应用程序显示若干状态消息。 按 ENTER 可以关闭该应用程序。  
  
## <a name="testing-the-workbook"></a>测试此工作簿  
 当你打开工作簿，<xref:Microsoft.Office.Tools.Excel.ListObject>现在显示已添加到缓存的数据集使用控制台应用程序的数据。  
  
#### <a name="to-test-the-workbook"></a>若要测试此工作簿  
  
1.  如果它仍处于打开状态，请关闭 Visual Studio 设计器中中的 AdventureWorksReport 工作簿。  
  
2.  在文件资源管理器，打开位于生成文件夹中的 AdventureWorksReport 工作簿**AdventureWorksReport**项目。 默认情况下，生成文件夹均位于以下位置之一：  

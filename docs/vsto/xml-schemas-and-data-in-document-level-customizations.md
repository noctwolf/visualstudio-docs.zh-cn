...
## <a name="deployment-of-solutions-that-include-xml-schemas"></a>包含 XML 架构的解决方案的部署  
 应创建的安装程序来部署使用映射到文档的 XML 架构的解决方案。 安装程序应在用户的计算机上的架构库中注册该架构。 如果不注册该架构，因为 Word 生成基于用户打开它时在文档中的元素的临时架构仍然有效解决方案。 但是，用户将无法再执行验证或保存用于创建项目的架构。 有关安装程序的详细信息，请参阅[部署应用程序、 服务和组件](/visualstudio/deployment/deploying-applications-services-and-components)。  

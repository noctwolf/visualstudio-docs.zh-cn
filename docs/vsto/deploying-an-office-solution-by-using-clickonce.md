...
##  <a name="Put"></a> 将解决方案文档置于最终用户的计算机 （仅文档级自定义） 上  
 ...
 下面的过程假定项目名称即**ExcelWorkbook**并将其发布到解决方案**C:\publish**目录在计算机上。  
...
1.  打开**c:\publish**使用目录**文件资源管理器**。  
  
2.  打开**应用程序文件**文件夹，，然后打开对应于最新的文件夹已发布版本的解决方案。  
  
3.  打开**ExcelWorkbook.dll.manifest**记事本等文本编辑器中的文件。  
  
4.  在 `</vstav3:update>` 元素之后添加以下代码。 类特性`<vstav3:entryPoint>`元素中，使用以下语法： *NamespaceName.ClassName*。 在以下示例中，命名空间和类名称相同，因此生成的入口点名称为 `FileCopyPDA.FileCopyPDA`。  
...
##  <a name="Custom"></a> 创建自定义安装程序  
 为 Office 解决方案，而不是使用当你发布解决方案时为你创建的安装程序，可以创建自定义安装程序。 例如，可以使用登录脚本来启动安装，或者使用批处理文件来安装解决方案，而无需用户交互。 如果最终用户计算机上已安装了系统必备组件，则这些方案都能顺利实现。  
...
|参数|定义|  
|---------------|----------------|  
|/Install 或 /I|安装解决方案。 此选项必须后跟部署清单的路径。 您可以在本地计算机上指定路径，一个通用命名约定 (UNC) 文件共享。 可以指定本地路径 (*C:\FolderName\PublishFolder*)，相对路径 (*发布\\*)，或完全限定的位置 (*\\\ServerName\FolderName*或 http://*ServerName/FolderName*)。|  
|/Uninstall 或 /U|卸载解决方案。 此选项必须后跟部署清单的路径。 您可以指定可以在本地计算机上的路径，一个 UNC 文件共享。 可以指定本地路径 (*c:\FolderName\PublishFolder*)，相对路径 (*发布\\*)，或完全限定的位置 (*\\\ServerName\FolderName*或 http://*ServerName/FolderName*)。|  

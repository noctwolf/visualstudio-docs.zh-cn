...
-   [Office 运行时的 visual Studio 工具](#VisualStudioToolsForOfficeRuntime)  
...
### <a name="temporary-certificates"></a>临时证书  
 ...
 该临时证书是在首次生成 Office 项目之后生成的。 下次按**F5**，因为项目将被标记为已更改时添加证书后，会重新生成项目。  
  
 在经过一段时间后，可能会有很多临时证书，因此应不定期地清除临时证书。  
  
##  <a name="VisualStudioToolsForOfficeRuntime"></a> Office 运行时的 visual Studio 工具  
...
### <a name="sequence-of-security-checks-during-installation"></a>在安装过程中的安全检查的序列  
 ...
 可以四种方式之一来启动安装过程： 通过运行安装程序、 打开部署清单、 打开 Microsoft Office 应用程序宿主，或通过运行*VSTOInstaller.exe*。  
  
 第一项安全检查仅适用于文档级解决方案。 文档级解决方案的文档必须位于可信位置。 如果文档位于远程网络文件共享，或者具有 *.doc*或 *.docm*文件扩展名，必须将文档的位置添加到受信任的位置列表。 有关详细信息，请参阅[向文档授予信任](../vsto/granting-trust-to-documents.md)。  
...
## <a name="see-also"></a>请参阅  
 ...
 [通过使用包含列表信任 Office 解决方案](../vsto/trusting-office-solutions-by-using-inclusion-lists.md)   

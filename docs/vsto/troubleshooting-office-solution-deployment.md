...
## <a name="change-the-assembly-name-causes-conflicts"></a>更改程序集名称会导致冲突  
 如果你更改**程序集名称**中的值**应用程序**页**项目设计器**你已部署解决方案后，将修改发布工具安装程序包，以具有一个*Setup.exe*文件和两个部署清单。 如果部署两个清单文件，则可能会出现以下条件：  
...
## <a name="check-for-updates-takes-a-long-time"></a>检查更新需要很长时间  
 Visual Studio 2010 Tools for Office runtime 提供管理员可用来将下载清单和解决方案的超时值设置一个注册表项。  
...
## <a name="cant-update-or-publish-to-a-network-file-share"></a>无法更新或发布到网络文件共享  
 网络文件共享上的 office 解决方案可能会在更新期间显示误导性消息，如果解决方案的*Setup.exe*在发布更新时，文件被锁定的进程中。 该消息可能会显示以下内容：“无法将‘setup.exe’添加到 Web。 此 Web 中已存在文件‘setup.exe’。”  
...
## <a name="publish-using-localhost-can-cause-installation-problems"></a>发布使用 Localhost 可能会导致安装问题  
...
## <a name="cached-assemblies-are-loaded-instead-of-updated-assemblies"></a>缓存程序集都会加载而不是更新的程序集  
...
## <a name="installation-fails-when-the-uri-has-characters-that-arent-us-ascii"></a>安装失败时 URI 具有不属于 US-ASCII 字符  
...
## <a name="uncaught-exception-or-method-not-found-error-when-you-install-a-solution"></a>未捕获的异常或方法时，无法找到错误安装解决方案  
 当通过打开部署清单来安装 Office 解决方案 ( *.vsto*文件)，可能会出现以下条件的 Office 应用程序、 文档或工作簿，错误消息：  
...
## <a name="manifest-registry-keys-for-add-ins-change-after-an-installshield-limited-edition-project-is-built"></a>生成 InstallShield Limited Edition 项目后清单外接程序更改的注册表项  
 是 VSTO 外接程序安装的一部分的清单注册表项有时程序从更改 *.vsto*到 *。 dll.manifest*生成 InstallShield Limited Edition 项目时。  
  
 要解决此问题，请在另一个解决方案中创建 InstallShield Limited Edition 项目，或使用 CompanyName.AddinName 作为包含 VSTO 外接程序名称的注册表项的值。  
  
## <a name="the-clickonce-installer-for-your-office-solution-doesnt-install-the-primary-interop-assemblies"></a>为 Office 解决方案 ClickOnce 安装程序不安装主互操作程序集  
 运行 ClickOnce 为 Office 解决方案创建的安装程序时，Office 主互操作程序集 (PIA) 的安装程序仅当尚未安装任何 PIA 时才会运行。  
  
 如果安装程序未正确安装 Pia，来手动安装它们运行名为安装程序文件*o2007pia.msi*从安装目录。  
...
## <a name="cant-install-a-clickonce-solution-by-opening-the-deployment-manifest-from-the-web"></a>无法通过从 web 打开部署清单来安装 ClickOnce 解决方案  
 用户可以通过从 Web 打开部署清单来安装 Office 解决方案。 但是，某些安装 Internet 信息服务 (IIS) 块 *.vsto*文件扩展名。 在使用它来部署 Office 解决方案之前，必须在 IIS 中定义的 MIME 类型。  
...
## <a name="see-also"></a>请参阅  
 [排除 ClickOnce 部署问题](/visualstudio/deployment/troubleshooting-clickonce-deployments)   

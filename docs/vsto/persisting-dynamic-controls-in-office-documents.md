...
ms.locfileid: "34692790"
---
# <a name="persist-dynamic-controls-in-office-documents"></a>保留在 Office 文档中的动态控件
...
## <a name="persist-host-controls-in-the-document"></a>保留在文档中的主机控件  
...
## <a name="persist-windows-forms-controls-in-documents"></a>保留在文档中的 Windows 窗体控件  
...
1.  保存或关闭文档时，存储关于大小、位置和控件状态的信息。 文档级自定义项，你可以将数据保存到文档中的数据缓存。 在 VSTO 外接程序中，可以将数据保存到文档中的自定义 XML 部件中。  
...
#### <a name="remove-activex-wrappers-when-the-document-is-opened"></a>打开文档时删除 ActiveX 包装  
 若要删除所有 ActiveX 包装，调用`GetVstoObject`方法生成的主机项<xref:Microsoft.Office.Interop.Word.Document>或<xref:Microsoft.Office.Interop.Excel.Workbook>表示新打开的文档。 例如，若要从 Word 文档中删除所有 ActiveX 包装，可以调用`GetVstoObject`方法生成的主机项<xref:Microsoft.Office.Interop.Word.Document>传递给事件处理程序对象<xref:Microsoft.Office.Interop.Word.ApplicationEvents4_Event.DocumentOpen>事件。  
  
 知道只能在安装 VSTO 外接程序的计算机上打开文档时，此过程非常有用。 如果该文档可能会传递给其他未安装 VSTO 外接程序的用户，请考虑在关闭文档之前删除控件。  
  
 下面的代码示例演示如何调用`GetVstoObject`方法文档何时被打开。  
  
 [!code-vb[Trin_WordAddInDynamicControls#11](../vsto/codesnippet/VisualBasic/trin_wordaddindynamiccontrols/ThisAddIn.vb#11)]
 [!code-csharp[Trin_WordAddInDynamicControls#11](../vsto/codesnippet/CSharp/Trin_WordAddInDynamicControls/ThisAddIn.cs#11)]  
  
 尽管`GetVstoObject`方法主要用于生成新的主机项在运行时，此方法就是特定文档的第一个时间也会清除从文档的所有 ActiveX 包装。 有关如何使用`GetVstoObject`方法，请参阅[扩展 Word 文档和 Excel VSTO 外接程序在运行时中的工作簿](../vsto/extending-word-documents-and-excel-workbooks-in-vsto-add-ins-at-run-time.md)。  
  
 请注意，是否 VSTO 外接程序中创建动态控件，打开文档时，VSTO 外接程序中将已调用`GetVstoObject`作为创建控件的过程的一部分的方法。 不需要添加对的单独调用`GetVstoObject`方法以在此方案中删除 ActiveX 包装。  
  
#### <a name="remove-the-dynamic-controls-before-the-document-is-closed"></a>在关闭文档之前删除动态控件  

...
2.  在 C# 中，您必须将附加到的事件处理程序<xref:System.Windows.Forms.UserControl.Load>事件。 您可以将此代码放置在`ActionsControl`构造函数中的，在调用`InitializeComponent`。 有关如何创建事件处理程序的详细信息，请参阅[如何： 在 Office 项目中创建事件处理程序](../vsto/how-to-create-event-handlers-in-office-projects.md)。  
